...
core_path_must_exists = Gegeven pad moet bestaan, waarbij { $pad } wordt genegeerd
core_must_be_directory_or_file = Gegeven pad moet verwijzen naar een geldige directory of bestand, waarbij { $pad } wordt genegeerd.
core_excluded_paths_pointless_slash = Uitsluiten / is zinloos, omdat het betekent dat er geen bestanden zullen worden gescand
core_paths_unable_to_get_device_id = Kan geen apparaat-id verkrijgen uit de map { $pad }
...
core_failed_to_check_process_status = Faal bij het controleren van de status van het proces: { $reden }
core_failed_to_wait_for_process = Faalde bij wachten op proces: { $reden }
core_failed_to_read_video_properties = Faal bij het lezen van videogegevens: { $reden }
core_failed_to_execute_ffmpeg = Faalde om ffmpeg uit te voeren: { $reden }
core_ffmpeg_failed_with_status = ffmpeg faalde met status { $status }: { $stderr } (command: { $command })
core_failed_to_load_image_frame = Faillende laad van beeldframe: { $reden }
...
core_image_zero_dimensions = Afbeelding heeft nul breedte of hoogte "{ $pad }"
...
core_error_comparing_fingerprints = Fout bij het vergelijken van vingerafdrukken: { $reden }
