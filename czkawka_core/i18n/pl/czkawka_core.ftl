...
core_error_removing = Błąd podczas usuwania "{ $plik }": { $error }
...
core_video_width_exceeds_limit = Video szerokość { $width } przekracza limit { $limit }
core_video_height_exceeds_limit = Wideo wysokość { $height } przekracza limit { $limit }
...
core_frame_dimensions_mismatch = Wymiary kramy dla znacznik czasowego { $timestamp } nie pasują do wymiarów pierwszej kramy ({ $first_w }x{ $first_h })
core_failed_to_load_data_from_cache = Nie udało się załadować danych z pliku pamięci podręcznej { $file }, powód { $reason }
core_failed_to_load_data_from_json_cache = Nie udało się załadować danych z pliku pamięci podręcznej json { $file }, powód { $reason }
core_failed_to_replace_with_optimized = Nie udało się zastąpić pliku "{ $file }" zoptymalizowaną wersją: { $reason }
core_failed_to_write_data_to_cache = Nie można zapisać danych do pliku pamięci podręcznej "{ $file }", powód { $reason }
core_properly_saved_cache_entries = Prawidłowo zapisano do pliku { $count } wpisów w pamięci podręcznej.
core_video_processing_stopped_by_user = Przetwarzanie wideo zostało zatrzymane przez użytkownika
core_thumbnail_generation_stopped_by_user = Generowanie miniatur zostało zatrzymane przez użytkownika
...
core_cannot_create_config_folder = Nie można utworzyć folderu konfiguracyjnego "{ $folder }", powód { $reason }
core_cannot_create_cache_folder = Nie można utworzyć folderu pamięci podręcznej "{ $folder }", powód { $reason }
core_cannot_create_or_open_cache_file = Nie można utworzyć ani otworzyć pliku pamięci podręcznej "{ $file }", powód { $reason }
core_cannot_set_config_cache_path = Nie można ustawić ścieżki do konfiguracji/bufora - konfiguracja i bufor nie zostaną użyte.
core_invalid_extension_contains_space = { $extension } nie jest prawidłową rozszerzeniem, ponieważ zawiera puste miejsce w środku
core_invalid_extension_contains_dot = { $extension } nie jest prawidłową rozszerzeniem, ponieważ zawiera kropkę wewnątrz
