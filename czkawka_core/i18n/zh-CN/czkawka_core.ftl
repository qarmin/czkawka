...
core_file_modified_before_epoch = 文件 { $name } 似乎在 Unix Epoch 之前被修改过
core_folder_modified_before_epoch = 文件夹 { $name } 似乎在 Unix Epoch 之前被修改过
...
core_ffmpeg_not_found = 找不到正确安装FFmpeg的程序，这是您需要手动安装的外部程序。
