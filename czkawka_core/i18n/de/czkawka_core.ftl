...
core_path_must_exists = Der angegebene Pfad muss existieren, wobei { $pfad } ignoriert wird.
core_must_be_directory_or_file = Der angegebene Pfad muss auf ein gültiges Verzeichnis oder eine Datei verweisen, { $pfad } wird ignoriert.
core_excluded_paths_pointless_slash = Ausgeschlossen / ist zwecklos, weil es bedeutet, dass keine Dateien gescannt werden.
core_paths_unable_to_get_device_id = Kann den Gerätedatenbank-Eintrag aus dem Ordner { $pfad } nicht abrufen.
