# Core
core_similarity_original = Asıl
core_similarity_very_high = Çok Yüksek
core_similarity_high = Yüksek
core_similarity_medium = Orta
core_similarity_small = Düşük
core_similarity_very_small = Çok Düşük
core_similarity_minimal = Aşırı Düşük
core_cannot_open_dir = { $dir } dizini açılamıyor, nedeni: { $reason }
core_cannot_read_entry_dir = { $dir } dizinindeki girdi okunamıyor, nedeni: { $reason }
core_cannot_read_metadata_dir = { $dir } dizinindeki metaveri okunamıyor, nedeni: { $reason }
core_cannot_read_metadata_file = { $file } dosyasının metaverisi okunamıyor, nedeni: { $reason }
core_file_modified_before_epoch = { $name } dosyası Unix Epoch tarih formatından önce değiştirilmiş
core_folder_modified_before_epoch = { $name } klasörünü Unix Epoch tarih formatından önce değiştirilmiş
core_file_no_modification_date = { $name } dosyasının değişiklik tarihine erişilemiyor, nedeni: { $reason }
core_folder_no_modification_date = { $name } klasörünün değişiklik tarihine erişilemiyor, nedeni: { $reason }
core_cannot_start_scan_no_included_paths = Tarama başlatılamıyor, çünkü hiçbir dahil yol yok
core_skip_exist_check_all_included_paths_nonexistent = Tarama başlatılamıyor, çünkü tüm dahil yollar mevcut değil
core_missing_no_chosen_included_path = Geçerli bir dahil yol seçilemedi (hariç tutulan dosya yolları, tüm dahil yollarını hariç bırakmış olabilir)
core_reference_included_paths_same = Geçerli dahil dosya yolların tamamının referans dosya yolları olarak da referanslandırıldığı bir tarama başlatılamaz, lütfen doğrulamayı deneyin veya referans dosya yolları devre dışı bırakın
core_path_must_exists = Verilen dosya yolunun mevcut olması gerekmektedir, { $path } dikkate alınmıyor
core_must_be_directory_or_file = Verilen dosya yolu geçerli bir dizine veya dosyaya işaret etmelidir, { $path } dikkate alınmıyor
core_excluded_paths_pointless_slash = / dizinini hariç tutmak anlamsızdır, çünkü bu, hiçbir dosyanın taranmayacağı anlamına gelir
core_paths_unable_to_get_device_id = { $path } klasöründen cihaz kimliği elde edilemiyor
core_excluded_items_wildcard_required = Hariç Tutulan Öğeler Uyarısı: İfade içinde joker karakter * kullanılması gereklidir, { $expression } ifadesi dikkate alınmayacaktır
core_needs_allowed_extensions_limited_by_tool = Bu araçta bulunan tüm uzantılar ({ $extensions }) taramadan hariç tutulduğunda tarama başlatılamıyor
core_needs_allowed_extensions = Tüm uzantılar taramadan hariç tutulduğunda tarama başlatılamıyor
core_needs_to_set_at_least_one_broken_option = Bozuk ayar seçiliyken tarama başlatılamıyor
core_needs_to_set_at_least_one_bad_name_option = Bozuk bir isim seçeneği ayarlanmamışken tarama başlatılamıyor
core_ffmpeg_not_found = FFmpeg veya FFprobe için uygun bir kurulum bulunamıyor. Bunlar, manuel olarak kurulması gereken harici programlardır.
core_ffmpeg_not_found_windows = ffmpeg.exe ve ffprobe.exe PATH değişkenine eklenmiş veya uygulama programın çalıştırma klasöründe aynı yerde olduğundan emin olun
core_invalid_symlink_infinite_recursion = Sonsuz tekrarlama
core_invalid_symlink_non_existent_destination = Var olmayan hedef dosya
core_messages_limit_reached_characters = Mesaj sayısı belirlenen sınırı aştı ({ $current }/{ $limit } karakter), bu nedenle çıktı kesildi. Çıktının tamamını okumak için ayarlardan sınırlama seçeneğini devre dışı bırakın.
core_messages_limit_reached_lines = Mesaj sayısı belirlenen sınırı aştı ({ $current }/{ $limit } satır), bu nedenle çıktı kısaltıldı. Çıktının tamamını okumak için ayarlardan sınırlama seçeneğini devre dışı bırakın.
core_error_moving_to_trash = { $file } dosyası çöp kutusuna taşınamadı: { $error }
core_error_removing = "{ $file }" silinirken hata: { $error }
core_hardlink_unsupported_filesystem = "{ $path }" konumundaki dosya sistemi, hard link desteklemiyor ({ $reason })
core_no_similarity_method_selected = Benzerlik metodu seçili olmadan benzer müzik dosyası bulunamaz
core_failed_to_spawn_command = Komutun oluşturulması başarısız oldu: { $reason }
core_failed_to_check_process_status = İşlem durumunu kontrol edemedi: { $reason }
core_failed_to_wait_for_process = İşlem beklemede başarısız oldu: { $reason }
core_failed_to_read_video_properties = Video özellikleri okunamıyor: { $reason }
core_failed_to_execute_ffmpeg = ffmpeg'i çalıştırması başarısız: { $reason }
core_ffmpeg_failed_with_status = ffmpeg başarısız oldu { $status }: { $stderr } (komut: { $command })
core_failed_to_load_image_frame = Resim çerçevesi yüklenemedi: { $reason }
core_failed_to_extract_frame = { $time } saniyede kareyi "{ $file }" dosyasından çıkarılamadı: { $reason }
core_failed_to_save_thumbnail = "{ $file }" için önizlemeyi kaydedemedi: { $reason }
core_failed_get_frame_at_timestamp = { $timestamp } zaman damgası üzerinde çerçeve alınamadı "{ $file }": { $reason }
core_failed_get_frame_from_file = "{ $file }" adlı dosyadaki çerçeve alınamadı zaman damgası { $timestamp }'te: { $reason }
core_invalid_crop_rectangle = Geçersiz tarım dikdörtgeni: sol={ $left }, üst={ $top }, sağ={ $right }, alt={ $bottom }
core_failed_to_crop_video_file = "{ $file }" video dosyasını kırpmakta bir sorun oluştu: { $reason }
core_cropped_video_not_created = Kesilmiş video dosyası oluşturulmadı: { $temp }
core_unable_check_hash_of_file = Dosyanın hash'ini "{ $file }" kontrol edilemedi, nedeni { $reason }
core_error_checking_hash_of_file = Dosya "{ $file }" için hash kontrolünde hata oluştu, nedeni { $reason }
core_image_zero_dimensions = Görüntü sıfır genişliğe veya yüksekliğe sahiptir "{ $path }"
core_image_open_failed = "{ $path }" adlı görüntü dosyasını açamıyoruz: { $reason }
core_not_directory_remove = "{ $path }$" adlı klasörü silmeye çalışıyor, bu bir dizin değil
core_cannot_read_directory = "{ $path }" dizinini okuyamıyor
core_cannot_read_entry_from_directory = "{ $path }" dizininden girişi okuyamaz
core_folder_contains_file_inside = Klasör içinde "{ $entry }" dosyası "{ $folder }" içinde bulunmaktadır
core_unknown_directory_entry = Dosya türünü "{ $entry }" girişine ait "{ $path }" içinde belirleyemiyorum
core_video_width_exceeds_limit = Video genişliği { $width } limiti { $limit } değerini aşmaktadır
core_video_height_exceeds_limit = Video yüksekliği { $height } limiti { $limit }'i aşmaktadır
core_failed_to_process_video = Video dosyasını işleme sırasında bir hata oluştu: { $file } - Neden: { $reason }
core_optimized_file_larger = Optimize edilmiş dosya { $optimized } (boyut: { $new_size }) orijinal { $original } (boyut: { $original_size })'den daha küçük değil
core_unknown_codec = Bilinmeyen codec: { $codec }
core_invalid_video_optimizer_mode = Geçersiz video optimizasyon modu: '{ $mode }'. İzin verilen değerler: transcode, crop
core_folder_does_not_exist = Klasör bulunamadı: { $folder }
core_path_not_directory = Yol bir dizin değildir: { $folder }
core_test_error_for_folder = Dosya hatası için klasör: { $folder }
core_unknown_exif_tag_group = Bilinmeyen EXIF etiket grubu: { $tag }
core_error_comparing_fingerprints = Parmak izlerini karşılaştırmada hata: { $reason }
core_failed_to_generate_thumbnail_frames_different_dimensions = "{ $file }" için önizlemeyi oluşturulamadı: Çıkarılan kareler farklı boyutlarda
core_failed_to_generate_thumbnail = "{ $file }" için önizlemeyi oluşturulamadı: { $reason }
core_failed_to_extract_frame_at_seek_time = { $time } saniyede kareyi "{ $file }" dosyasından çıkarılamadı: { $reason }
core_video_file_does_not_exist = Video dosyası mevcut değil (tarama/daha sonra adımları arasında kaldırılabilir): "{ $path }"
core_image_too_large = Görüntü çok büyü ({ $width }x{ $height }) - desteklenmeyen { $max } pikselden fazla
core_image_file_too_large = Resim dosyası çok büyük ({ $size } bayt) - { $limit } MB sınırı aşıyor
core_failed_to_get_video_metadata = Video meta verilerini dosyayı "{ $file }" için elde edilemedi: { $reason }
core_failed_to_get_video_codec = Dosya "{ $file }" için video codec'i alınamadı
core_failed_to_get_video_duration = Video süresini "{ $file }" dosyası için elde edilemedi
core_failed_to_get_video_dimensions = Video boyutlarını dosya için "{ $file }" alınamadı
core_frame_dimensions_mismatch = Çerçeve boyutları { $timestamp } zaman damgası için ilk çerçeve boyutlarıyla (%{ $first_w }x%{ $first_h }) uyuşmuyor
core_failed_to_load_data_from_cache = Verilen dosyadaki { $file } verisi yüklenemedi, nedeni { $reason }
core_failed_to_load_data_from_json_cache = JSON önbellek dosyasından veri yüklenemedi: { $file }, nedeni: { $reason }
core_failed_to_replace_with_optimized = Dosya "{ $file }" optimize edilmiş versiyon ile değiştirilemedi: { $reason }
core_failed_to_write_data_to_cache = Katalog dosyasına "{ $file }" yazamıyor, nedeni { $reason }
core_properly_saved_cache_entries = Doğru şekilde dosyaya { $count } önbellek girişi kaydedildi.
core_properly_loaded_cache_entries = Başarıyla yüklenen { $count } adet önbellek girdisi.
core_video_processing_stopped_by_user = Video işleme kullanıcının tarafından durduruldu
core_thumbnail_generation_stopped_by_user = Miniature oluşturma durduruldu kullanıcı tarafından
core_failed_to_optimize_video = Video "{ $file }" optimizasyonu başarısız: { $reason }
core_failed_to_crop_video = Video kırpma başarısız: "{ $file }": { $reason }
core_failed_to_get_metadata_of_optimized_file = Optimizasyonlu dosya "{ $file }": { $reason } meta verisi alınamadı
core_cannot_create_config_folder = Konfigürasyon klasörü "{ $folder }" oluşturulamazdı, nedeni { $reason }
core_cannot_create_cache_folder = "{ $folder }" önbellek klasörü oluşturulamaz, nedeni { $reason }
core_cannot_create_or_open_cache_file = "{ $file }" adlı geçici dosyayı oluşturamadı veya açamadı, nedeni: { $reason }
core_cannot_set_config_cache_path = Config/cache yolu yapılamadı - config ve cache kullanılmayacak.
core_invalid_extension_contains_space = { $extension } geçerli bir uzantı değildir çünkü içinde boşluk içermektedir
core_invalid_extension_contains_dot = { $extension } geçerli bir uzantı değildir çünkü içinde nokta içeriyor
core_ffmpeg_unknown_encoder = { $file } dosyasını { $encoder } kodlayıcı ile kodlayamaz. Mevcut FFmpeg sürümü bu kodlayıcıyı desteklemiyor. Gerekli codec desteğine sahip farklı bir FFmpeg sürümü kullanın veya başka bir kodlayıcı seçin.
core_ffmpeg_error = FFmpeg işleme sırasında { $file } dosyasında bir hata oluştu, hata kodu: { $code }, sebep: { $reason }
core_custom_command_missing_path_placeholder = Custom FFmpeg command must contain { "{PATH}" } as input file placeholder
core_custom_command_empty = Özel FFmpeg komutu boş olamaz
# Scan stage labels
# Collecting / scanning files (the { $entries_checked } counter grows while the disk is walked)
stage_collecting_files = { $entries_checked } dosya taranıyor
stage_collecting_folders = { $entries_checked } klasör taranıyor
stage_scanning_name = { $entries_checked } dosyanın adı taranıyor
stage_scanning_size_name = { $entries_checked } dosyanın boyutu ve adı taranıyor
stage_scanning_size = { $entries_checked } dosyanın boyutu taranıyor
# Cache load/save (indeterminate, no counters)
stage_filtering_outdated_cache = Eski önbellek girişlerini filtreleme
stage_loading_prehash_cache = Prehash önbelleği yükleniyor
stage_saving_prehash_cache = Prehash önbelleği kaydediliyor
stage_loading_hash_cache = Hash önbelleği yükleniyor
stage_saving_hash_cache = Hash önbelleği kaydediliyor
stage_loading_tags_cache = Etiket önbelleği yükleniyor
stage_saving_tags_cache = Etiket önbelleği kaydediliyor
stage_loading_fingerprints_cache = Parmak izi önbelleği yükleniyor
stage_saving_fingerprints_cache = Parmak izi önbelleği kaydediliyor
stage_loading_exif_cache = Yükleniyor EXIF önbelleği
stage_saving_exif_cache = EXIF önbelleğini kaydetme
# Per-tool work stages - items_stats holds "checked/total", size_stats holds the byte sizes
stage_hiding_links = Gizli bağlantıları ({ $items_stats }) gizle
stage_analyzed_partial_hash = { $items_stats } dosyanın kısmi hash’i analiz edildi ({ $size_stats })
stage_analyzed_full_hash = { $items_stats } dosyanın tam hash’i analiz edildi ({ $size_stats })
stage_checked_tags = { $items_stats } öğesinin etiketleri kontrol edildi
stage_compared_tags = { $items_stats } öğesinin etiketleri karşılaştırıldı
stage_checked_content = { $items_stats } öğesinin içeriği kontrol edildi ({ $size_stats })
stage_compared_content = { $items_stats } öğesinin içeriği karşılaştırıldı
stage_hashed_images = Hashed { $items_stats } resimler({ $size_stats })
stage_compared_image_hashes = { $items_stats } karşılaştırmalı görüntü-hash'leri
stage_hashed_videos = Hashlanmış { $items_stats } videolar
stage_created_thumbnails = { $items_stats } video için küçük resimler oluşturduğun
stage_checked_videos = Kontrol edildi { $items_stats } videoları ({ $size_stats })
stage_extracted_exif_tags = Çıkarılan EXIF etiketleri { $items_stats } dosyalarından ({ $size_stats })
stage_checked_files = { $items_stats } dosya kontrol edildi ({ $size_stats })
stage_checked_files_bad_extensions = Yanlış uzantılı { $items_stats } dosya kontrol edildi
stage_checked_files_bad_names = Yanlış uzantılı { $items_stats } dosya kontrol edildi
stage_checking_empty_files_content = { $items_stats } dosyalarının içeriğini kontrol ediliyor ({ $size_stats })
# File operations (delete/rename/move/...); the "no_size" variant is used when no byte total is known
stage_deleting_files = { $items_stats } dosyalar siliniyor ({ $size_stats })
stage_deleting_no_size_files = { $items_stats } dosyalar siliniyor
stage_renaming_files = { $items_stats } dosyalar yeniden adlandırılıyor
stage_moving_files = { $items_stats } dosyalar taşınıyor ({ $size_stats })
stage_moving_no_size_files = { $items_stats } dosyalar taşınıyor
stage_hardlinking_files = Hardlinking { $items_stats } dosyası ({ $size_stats })
stage_hardlinking_no_size_files = Sert Bağlantı { $items_stats } dosyası
stage_symlinking_files = Symlinking { $items_stats } dosyası ({ $size_stats })
stage_symlinking_no_size_files = Symlinking { $items_stats } dosyasını
stage_optimizing_videos = Optimize edilmiş { $items_stats } video ({ $size_stats })
stage_optimizing_no_size_videos = Optimize Edilmiş { $items_stats } videosu
stage_cleaning_exif = Temizleme EXIF'i { $items_stats } dosyasından ({ $size_stats })
stage_cleaning_no_size_exif = Temizleme EXIF'i { $items_stats } dosyasından
