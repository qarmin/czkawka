...
core_path_must_exists = Verilen yolun mevcut olması gerekir, { $yol }'u göz ardı ederek
...
core_failed_to_crop_video_file = Video dosyasını kırpma başarısızlandı: { $reason }
...
core_failed_to_process_video = Video dosyasını işlenemedi: { $reason }
...
core_failed_to_load_data_from_json_cache = JSON önbellek dosyasından ($file) veri yüklenemedi, nedeni { $reason }
...
core_cannot_create_or_open_cache_file = "{} $file" dosyasının oluşturulamıyor veya açılamıyor, sebep { $reason }
