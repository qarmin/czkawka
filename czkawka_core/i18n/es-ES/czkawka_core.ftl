...
core_failed_to_spawn_command = Falló al generar el comando: { $razón }
core_failed_to_check_process_status = Error al verificar el estado del proceso: { $razón }
core_failed_to_wait_for_process = Error al esperar al proceso: { $razón }
core_failed_to_read_video_properties = No se pudo leer las propiedades del video: { $razón }
core_failed_to_execute_ffmpeg = No se pudo ejecutar ffmpeg: { $razón }
core_ffmpeg_failed_with_status = ffmpeg falló con estado { $status }: { $stderr } (comando: { $command })
core_failed_to_load_image_frame = No se pudo cargar el marco de imagen: { $razón }
