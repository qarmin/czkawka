...
core_hardlink_unsupported_filesystem = 檔案系統位於 "{ $path }" 的位置，不支援硬連結 ({ $reason })
...
core_ffmpeg_unknown_encoder = 無法使用 { $encoder } 編碼器來編碼 { $file }。 目前的 FFmpeg 版本不支援此編碼器。 請使用另一個支援所需編碼器的 FFmpeg 版本，或選擇其他編碼器。.
...
stage_loading_exif_cache = 載入 EXIF 緩存
stage_saving_exif_cache = 儲存 EXIF 緩存
...
stage_optimizing_videos = 優化 { $items_stats } 影片 ({ $size_stats })
stage_optimizing_no_size_videos = 優化 { $items_stats } 影片
