...
core_missing_no_chosen_included_directory = حداقل یک مسیر目录必须提供
core_included_directory_zero_valid_directories = برایده دایرکتوری خطا: هیچ مسیر وارد شده‌ای که معتبر است، پیدا نشد که ضروری است
core_excluded_directory_pointless_slash = دایرکتوری‌ها: بیانگر عدم تضمن /\ مفید نیست، زیرا این به این معنی است که هیچ فایلی بازسازی نخواهد شد
core_directory_overlap = دایرکتوری‌ها: تمام دایرکتوری‌هایی که جستجو می‌شوند با دایرکتوری‌های از بین برده چسبیده هستند
core_ffmpeg_not_found = غیر از نسخه مناسب فاینگم، ناپیدا است. این یک برنامه بیرونیست که باید به صورت دستی نصب شود.
...
rust_error_moving_to_trash = خطا در منتقل کردن "{ $file }" به سبد حذف شد: { $error }
rust_error_removing = خطا در حذف "{ $file }": { $error }