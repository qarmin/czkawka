...
core_failed_to_check_process_status = Falha ao verificar o status do processo: { $razão }
core_failed_to_wait_for_process = Falhou ao aguardar o processo: { $razão }
core_failed_to_read_video_properties = Falha ao ler as propriedades do vídeo: { $razão }
core_failed_to_execute_ffmpeg = Falha ao executar ffmpeg: { $razão }
core_ffmpeg_failed_with_status = ffmpeg falhou com o status { $status }: { $stderr } (comando: { $command })
core_failed_to_load_image_frame = Falha ao carregar o quadro de imagem: { $razão }
...
core_video_width_exceeds_limit = Vídeo largura { $largura } excede o limite de { $limite }
...
core_optimized_file_larger = Arquivo otimizado { $otimizado } (tamanho: { $novo_tamanho }) não é menor que o original { $original } (tamanho: { $tamanho_original })
...
core_error_comparing_fingerprints = Erro ao comparar impressões digitais: { $razão }
...
core_image_too_large = A imagem é muito grande ({ $largura }x{ $altura }) - mais do que o suportado { $máximo } pixels
