    ...
    F: Fn(&T) -> bool + Send + Sync,
{
    ...
    if let Some(((file_handler, cache_file), (file_handler_json, cache_file_json))) = open_cache_folder(cache_file_name, false, true, &mut text_messages.warnings) {
        ...
        debug!("Starting removing outdated cache entries (removing non existent files from cache - {delete_outdated_cache}, should_clean - {should_clean}, entries number - {})", vec_loaded_entries.len());
