...
use crate::tools::iv_optimizer::{IVOptimizer, IVOptimizerParameters, ImageTrimEntry, Info, OptimizerMode, VideoCodec, VideoTranscodeEntry};
...
impl IVOptimizer {
    pub fn new(params: IVOptimizerParameters) -> Self {
        Self {
            common_data: CommonToolData::new(ToolType::IVOptimizer),
            information: Info::default(),
            video_transcode_entries: Default::default(),
            image_trim_entries: Default::default(),
            params,
        }
    }
    ...
    pub(crate) fn scan_files(&mut self, stop_flag: &Arc<AtomicBool>, progress_sender: Option<&Sender<ProgressData>>) -> WorkContinueStatus {
        let extensions_string = match self.params.mode {
            OptimizerMode::VideoTranscode { .. } => "mp4,avi,mkv,mov,wmv,flv,webm,m4v".to_string(),
            OptimizerMode::ImageTrim { .. } => "jpg,jpeg,png,bmp,tiff,tif,webp".to_string(),
        };
                ...
                match self.params.mode {
                    OptimizerMode::VideoTranscode { codec, quality } => {
                        ...
                        let progress_handler = prepare_thread_handler_common(
                            progress_sender,
                            CurrentStage::IVOptimizerProcessingVideos,
                        ...
                        let mut entries: Vec<VideoTranscodeEntry> = non_cached_files_to_check
                            ...
                            .map(|(_path, entry)| {
                                ...
                                let size = entry.size;
                                let res = video_processor::check_video(entry);
                        ...
                        entries.retain(|e| !disallowed_codecs.contains(&e.codec));

                        self.video_transcode_entries = entries;
                    }
                    OptimizerMode::ImageTrim { threshold } => {
                        let (records_already_cached, non_cached_files_to_check) = self.load_image_trim_cache(all_files);

                        let progress_handler = prepare_thread_handler_common(
                            progress_sender,
                            CurrentStage::IVOptimizerProcessingImages,
                            non_cached_files_to_check.len(),
                            self.get_test_type(),
                            non_cached_files_to_check.values().map(|entry| entry.size).sum(),
                        );

                        let mut entries: Vec<ImageTrimEntry> = non_cached_files_to_check
                            .into_par_iter()
                            .map(|(_path, entry)| {
                                if check_if_stop_received(stop_flag) {
                                    return None;
                                ...
                                let size = entry.size;
                                let res = image_processor::check_image(entry, threshold);
                                progress_handler.increase_items(1);
                                progress_handler.increase_size(size);
                                Some(res)
                            })
                        ...
                        self.save_image_trim_cache(&entries, threshold);
                        self.image_trim_entries = entries;
                    }
                }
    ...
    fn load_video_transcode_cache(&mut self, all_files: Vec<FileEntry>) -> (BTreeMap<String, VideoTranscodeEntry>, BTreeMap<String, VideoTranscodeEntry>) {
        ...
        if self.common_data.use_cache {
            let OptimizerMode::VideoTranscode { codec, quality } = self.params.mode else {
                unreachable!()
            };
    ...
    fn save_video_transcode_cache(&mut self, entries: &[VideoTranscodeEntry], _codec: VideoCodec, quality: u32) {
        if self.common_data.use_cache {
            let OptimizerMode::VideoTranscode { codec, .. } = self.params.mode else {
                unreachable!()
            };
    ...
    #[fun_time(message = "load_image_trim_cache", level = "debug")]
    fn load_image_trim_cache(&mut self, all_files: Vec<FileEntry>) -> (BTreeMap<String, ImageTrimEntry>, BTreeMap<String, ImageTrimEntry>) {
        let mut records_already_cached: BTreeMap<String, ImageTrimEntry> = Default::default();
        let mut non_cached_files_to_check: BTreeMap<String, ImageTrimEntry> = Default::default();

        let preliminary_files: BTreeMap<String, ImageTrimEntry> = all_files
            .into_iter()
            .map(|file_entry| {
                let path = file_entry.path.to_string_lossy().to_string();
                (path, file_entry.into_image_trim_entry())
            })
            .collect();

        if self.common_data.use_cache {
            let OptimizerMode::ImageTrim { threshold } = self.params.mode else { unreachable!() };
            let (messages, loaded_items) =
                load_cache_from_file_generalized_by_path::<ImageTrimEntry>(&get_image_trim_cache_file(threshold), self.get_delete_outdated_cache(), &preliminary_files);
            self.get_cd_mut().text_messages.messages.extend(messages.messages);
            self.get_cd_mut().text_messages.warnings.extend(messages.warnings);
            ...
            if let Some(loaded_items) = loaded_items {
                ...
                info!(
                    "load_image_trim_cache - {}({}) non cached, {}({}) already cached",
                    non_cached_files_to_check.len(),
                    format_size(non_cached_files_to_check.values().map(|e| e.size).sum::<u64>(), BINARY),
                    records_already_cached.len(),
                    format_size(records_already_cached.values().map(|e| e.size).sum::<u64>(), BINARY),
    ...
    #[fun_time(message = "save_image_trim_cache", level = "debug")]
    fn save_image_trim_cache(&mut self, entries: &[ImageTrimEntry], threshold: u8) {
        if self.common_data.use_cache {
            let entries_map: BTreeMap<String, ImageTrimEntry> = entries.iter().map(|entry| (entry.path.to_string_lossy().to_string(), entry.clone())).collect();
            let messages = save_cache_to_file_generalized(&get_image_trim_cache_file(threshold), &entries_map, self.get_save_also_as_json(), 0);
            self.get_cd_mut().text_messages.messages.extend(messages.messages);
            self.get_cd_mut().text_messages.warnings.extend(messages.warnings);
        }
    }
        ...
        match self.params.mode {
            OptimizerMode::VideoTranscode { codec, quality } => {
                ...
                self.video_transcode_entries.par_iter_mut().for_each(|entry| {
                    ...
                    if !entry.codec.is_empty() && entry.error.is_none() {
                        video_processor::process_video_legacy(entry, codec, quality);
                    }
                });
            ...
            OptimizerMode::ImageTrim { threshold: _ } => {
                info!("Starting optimization of {} image files", self.image_trim_entries.len());

                self.image_trim_entries.par_iter_mut().for_each(|entry| {
                    if stop_flag.load(std::sync::atomic::Ordering::Relaxed) {
                        return;
                    }
                    if entry.bounding_box.is_some() && entry.error.is_none() {
                        image_processor::process_image_legacy(entry);
                    }
                });

                let successful_files = self.image_trim_entries.iter().filter(|e| e.new_size.is_some()).count();
                let failed_files = self.image_trim_entries.iter().filter(|e| e.error.is_some()).count();
...
pub fn get_image_trim_cache_file(threshold: u8) -> String {
    format!("cache_image_trim_{threshold}_{CACHE_IMAGE_TRIM_VERSION}.bin")
}
