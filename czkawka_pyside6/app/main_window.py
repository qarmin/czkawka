"""Main application window for Czkawka PySide6 interface."""
...
class MainWindow(QMainWindow):
    ...
    def _on_tab_changed(self, tab: ActiveTab):
        ...
        self._status_label.setText(f"Tab: {tab.name.replace('_', ' ').title()}")
    ...
    def _start_scan(self):
        ...
        if not self._state.settings.included_paths:
            QMessageBox.warning(
                self, "No Directories",
                "Please add at least one directory to scan in the bottom panel."
            )
            return
    ...
    def _on_scan_error(self, error_msg: str):
        ...
        self._bottom_panel.set_text(f"Error: {error_msg}")
    ...
    def _show_settings(self):
        ...
        self._settings_panel.setWindowTitle("Czkawka Settings")
    ...
    def _create_hardlinks(self):
        ...
        if not reference:
            QMessageBox.warning(
                self, "No Reference",
                "Cannot determine reference file. Leave at least one file unchecked in the group."
            )
            return

        reply = QMessageBox.question(
            self, "Create Hardlinks",
            f"Replace {len(checked)} selected file(s) with hardlinks to:\n{reference}?",
            QMessageBox.Yes | QMessageBox.No
        )
        if reply == QMessageBox.Yes:
            created, errors = FileOperations.create_hardlinks(checked, reference)
            self._status_label.setText(f"Created {created} hardlink(s)")
            if errors:
                self._bottom_panel.set_text("\n".join(errors))
                self._bottom_panel.show_text()
    ...
    def _create_symlinks(self):
        ...
        reply = QMessageBox.question(
            self, "Create Symlinks",
            f"Replace {len(checked)} selected file(s) with symlinks to:\n{reference}?",
            QMessageBox.Yes | QMessageBox.No
        )
        if reply == QMessageBox.Yes:
            created, errors = FileOperations.create_symlinks(checked, reference)
            self._status_label.setText(f"Created {created} symlink(s)")
            if errors:
                self._bottom_panel.set_text("\n".join(errors))
                self._bottom_panel.show_text()
    ...
    def _rename_files(self):
        ...
        if tab == ActiveTab.BAD_EXTENSIONS:
            ...
            if dialog.exec() == RenameDialog.Accepted:
                ...
                self._status_label.setText("Extensions fixed" if success else f"Error: {msg}")
            ...
            if dialog.exec() == RenameDialog.Accepted:
                ...
                self._status_label.setText("Names fixed" if success else f"Error: {msg}")
    ...
    def _clean_exif(self):
        ...
        reply = QMessageBox.question(
            self, "Clean EXIF",
            f"Remove EXIF metadata from {len(checked)} selected file(s)?",
            QMessageBox.Yes | QMessageBox.No
        )
        if reply == QMessageBox.Yes:
            ...
            self._status_label.setText(f"Cleaned EXIF from {cleaned} file(s)")
            if errors:
                self._bottom_panel.set_text("\n".join(errors))
                self._bottom_panel.show_text()
    ...
    def _optimize_video(self):
        ...
        QMessageBox.information(
            self, "Video Optimization",
            f"Video optimization for {len(checked)} file(s) will be performed "
            "using czkawka_cli. Check the status bar for progress."
        )
        # Video optimization is done via CLI
        self._status_label.setText("Video optimization: use CLI directly for this feature")
