...
rust_cannot_load_preset = プリセット { $preset_idx }を読み込めません、理由： { $reason }
...
rust_error_creating_folder = フォルダ作成中にエラーが発生しました: { $error }
...
rust_hashed_images = { $items_stats } イメージのハッシュ化({ $size_stats })
rust_compared_image_hashes = { $items_stats } 画像ハッシュの比較
rust_hashed_videos = { $items_stats } ビデオのハッシュ化
...
rust_found_duplicate_files = { $items_found } 類似の重複ファイルが見つかりました
...
subsettings_images_ignore_same_size = 同じサイズを無視
...
subsettings_videos_ignore_same_size = 同じサイズを無視
...
subsettings_music_compare_fingerprints_only_with_similar_titles = 類似のタイトルとのみ比較
...
delete = アイテムを削除
delete_confirmation = 選択したアイテムを削除してもよろしいですか？
