...
rust_trash_confirmation_number_groups = { $items } 個のグループで { $groups } 個のアイテムが選択されました。
...
other_apps_video_thumbnailer_desc = Czkawakaで使用されているビデオサムネイルジェネレータの周りのWrapper。
other_apps_space_finder_desc = あなたのシステム上で最大のファイルのシンプルなファインダー。
...
selection_all_except_longest_path = 選択を解除する、最長パスを除く
selection_all_except_shortest_path = 選択を解除し、最短経路を除く
...
option_search_mode_smallest = 最も小さい
...
option_video_crop_type_black_bars = 黒い延べ棒
...
subsettings_videos_audio_preset_identical = 同じ動画
...
subsettings_videos_visual_preset_near_identical = 近くに同じ
subsettings_videos_visual_preset_similar = 似ています
subsettings_videos_visual_preset_movies = 映画 (長いスキップ)
subsettings_videos_duration_tolerance_pct = 持続時間許容差 [%]
subsettings_videos_min_matching_windows = 最小一致するウィンドウ
...
subsettings_empty_files_type = 検索する追加のファイル タイプ
...
subsettings_empty_files_non_printable_content_hint = また、null、space、tab、CR、LF、VT、FFなどの非空白文字のみで構成されるファイルも見つけます。.
...
subsettings_broken_files_video_ffmpeg_info = ffmpeg(フルデコイ)を使用したディープビデオチェック。ファイルが正常に再生されていても、非常に遅く、ペダント的なエラーが検出される可能性があります。.
...
settings_select_label_one_smallest_text = 最小（1）
...
settings_select_label_one_oldest_text = 古い順
...
settings_select_label_longest_text = 最も長い
...
subsettings_video_optimizer_custom_command_hint = 入力ファイルの Command with { "{PATH}" } が自動的に追加されます。.
subsettings_video_optimizer_hardware_encoder = ハードウェアエンコード
...
subsettings_temporary_files_extensions_text = エクステンション:
subsettings_temporary_files_extensions_hint_text = 一時的に扱われる拡張子/サフィックスのカンマ区切りリスト (例: .tmp,.bak,~) をリセットします。組み込みのデフォルトをリセットします。.
...
optimize_noise_reduction_hint = ノイズリダクションはエンコード時間を大幅に増加させる可能性があります。.
optimize_noise_reduction_strength_hint = 1 = 最小、10 = 最大ノイズリダクション。.
...
popup_custom_select_title_text = カスタムの選択 / 選択解除
...
popup_custom_hint_date_text = Date columns: DD-MM-YYYYまたは YYYY-MM-DD, optional time HH:MM:SS e.g. >= 2020-01-01 or < 31-12-2022 23:59:59
...
context_menu_exclude_item_text = 除外するアイテム
