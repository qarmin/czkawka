...
rust_no_similarity_method_selected = Impossibile trovare file musicali simili senza un metodo di somiglianza selezionato.
...
settings_similar_videos_image_preview = Anteprima immagine
...
popup_rename_message = Questo rinominerà le estensioni dei file selezionati in quelli più corretti
...
subsettings_videos_thumbnail_percentage = Posizione della miniatura nel video (%)
...
rust_checked_images = Controllate { $items_stats } immagini ({ $size_stats })
...
settings_application_scale_hint_text = Applicazione manuale della scala (ad es. 1.0 = 100%)
...
settings_similar_videos_generate_thumbnail_grid_instead_of_single_image_hint_text = Genera una griglia di anteprime invece di un'unica immagine (potrebbe richiedere più tempo)
settings_similar_videos_generate_thumbnail_grid_instead_of_single_image = Genera griglia miniature invece di singola immagine
settings_similar_videos_clear_unused_thumbnails = Elimina miniature video inutilizzate più vecchie di 7 giorni all'avvio dell'app
...
subsettings_video_optimizer_image_threshold = Soglia dell'immagine
...
optimize_text = Ottimizza le immagini
optimize_confirmation_text = Sei sicuro di voler ottimizzare gli elementi selezionati?
