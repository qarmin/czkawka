...
rust_init_error_title = Krytyczna Błąd Podczas Uruchamiania Aplikacji
rust_init_error_message = 
        Wystąpił krytyczny błąd podczas uruchamiania aplikacji:

        { $error_message }

        Może być spowodowany brakiem lub uszkodzonymi sterownikami OpenGL/Vulkan, uruchamianiem aplikacji w maszynie wirtualnej lub błędem w Krokiecie lub jednej z jego bibliotek.

        Możesz spróbować uruchomić różne wersje aplikacji (skia_opengl, skia_vulkan, femtovg_opengl - domyślna) lub z rendererem sprzętowym, aby sprawdzić, czy to rozwiązuje problem.
...
rust_no_included_paths = Nie można uruchomić skanu, gdy nie ustawiono żadnych ścieżek.
rust_all_paths_referenced = Nie można uruchomić skanu, gdy wszystkie ścieżki oznaczone są jako ścieżki referencyjnej(używanej do oznaczania plików tylko do odczytu), musisz wyłączyć pole wyboru przynajmniej jednej takiej ścieżki.
...
rust_found_bad_names = Znaleziono { $items_found } plików o złych nazwach w { $time }
...
rust_cache_size_reduced = Zmniejszono rozmiar plików pamięci podręcznej o { $size }
rust_cache_time_elapsed = Czas jaki upłynął: { $time }
...
subsettings_broken_files_video_info = Używa ffmpeg/ffprobe. Jest to dość wolne i może wykryć pedantyczne błędy nawet jeśli plik odtwarza się poprawnie.
subsettings_bad_names_issues = Sprawdzanie nazw plików
subsettings_bad_names_uppercase_extension = Wielkie litery rozszerzenie
subsettings_bad_names_uppercase_extension_hint = Znajduje pliki z literami wielkimi w rozszerzeniu (np. .JPG, .Mp3) i sugeruje wersję małą literami
subsettings_bad_names_emoji_used = Emoji w nazwie
subsettings_bad_names_emoji_used_hint = Znajduje pliki ze znakami emoji (😀, 🎉, itd.) w nazwie i sugeruje ich usunięcie
subsettings_bad_names_space_at_start_end = Spacje na początku/końcu
subsettings_bad_names_space_at_start_end_hint = Znajduje pliki z odstępami w początku lub końcu nazwy i sugeruje przycięcie ich
subsettings_bad_names_non_ascii = Znaki różne niż Ascii
subsettings_bad_names_non_ascii_hint = Znajduje znaki nie-ASCII (ą, ć, ñ, itd.) i sugeruje ich zastąpienie odpowiednikami ASCII (a, c, n) lub usunięcie, jeśli nie istnieje mapa.
subsettings_bad_names_restricted_charset = Ograniczony zbiór znaków
subsettings_bad_names_restricted_charset_hint = Transliteruje znaki nie-ASCII na ASCII, a następnie znajduje pliki zawierające znaki spoza zakresu 0-9a-zA-Z i znaków dopuszczonych przez użytkownika.
subsettings_bad_names_allowed_chars = Dozwolone znaki
subsettings_bad_names_remove_duplicated = Powtórzone znaki
...
settings_delete_outdated_cache_entries = Usuń automatycznie nieaktualne wpisy pamięci podręcznej
settings_delete_outdated_cache_entries_hint = Gdy jest włączone, aplikacja będzie weryfikować podczas ładowania pamięci podręcznej (nie więcej niż raz na tydzień), czy wpisy w pamięci podręcznej nadal wskazują na istniejące i niezmienione pliki/dane.
...
settings_application_scale_hint_text = Gdy włączona jest ręczna skala, umożliwia to wybranie niestandardowego współczynnika skali, ale całkowicie wyłącza automatyczną skalę opartą na DPI monitora.
...
settings_video_thumbnails_preview = Próbka obrazu
...
settings_video_thumbnails_header = Zdjęcia miniatur wideo
settings_video_thumbnails_generate = Generuj miniaturki
settings_video_thumbnails_position = Thumbnail pozycja wideo (%)
settings_video_thumbnails_generate_grid = Wygeneruj siatkę miniatur zamiast pojedynczego obrazu
settings_video_thumbnails_generate_grid_hint = Generowanie wielu obrazów w siatce jest znacznie wolniejsze niż generowanie pojedynczego miniaturki.
settings_video_thumbnails_grid_tiles_per_side = Liczba płytek na boku w siatce miniatur
settings_video_thumbnails_grid_tiles_per_side_hint = Liczba miniatur w rzędzie w siatce. Na przykład, wybranie 2 tworzy siatkę 2 x 2, co skutkuje pojedynczą miniaturą składającą się z 4 obrazów.
...
settings_play_audio_on_scan_completion_text = Odtwórz dźwięk, gdy skan zakończy się pomyślnie
settings_audio_feature_hint_text = Dostępne tylko podczas kompilacji z funkcją audio
settings_audio_env_variable_hint_text = Dźwięk może być zmieniony, ustawiając zmienną środowiskową KROKIET_AUDIO_STOP_FILE na poprawną ścieżkę do pliku audio
...
popup_new_paths_title = Proszę dodać ścieżki po jednej na linię
...
popup_clean_cache_confirmation_text = Czy na pewno chcesz wyczyścić przestarzałe wpisy pamięci podręcznej? To usunie wpisy pamięci podręcznej dla plików, które nie istnieją lub zostały zmodyfikowane.
popup_clean_cache_progress_text = Przetwarzanie pliku pamięci podręcznej:
popup_clean_cache_current_file_text = Obecny plik:
popup_clean_cache_file_progress_text = Obecny postęp pliku:
popup_clean_cache_overall_progress_text = Ogólny postęp:
...
subsettings_video_optimizer_crop_type = Typ Sortu
subsettings_video_optimizer_black_pixel_threshold = Czarny Pixelowy Próg
subsettings_video_optimizer_black_pixel_threshold_hint = Maksymalna wartość RGB dla każdego kanału piksela ma być uważana za czarny (0-128). Domyślnie: 20
subsettings_video_optimizer_black_bar_min_percentage = Czarny Pasek Minimalna Procenta
subsettings_video_optimizer_black_bar_min_percentage_hint = Minimal procent pikseli czarnego w wierszu/kolumnie, aby być uznanym za linię czarną (50-100). Domyślna: 90
subsettings_video_optimizer_max_samples = Max Przykłady
subsettings_video_optimizer_max_samples_hint = Maksymalna liczba klatek do analizy na wideo (5-1000). Domyślna: 60
subsettings_video_optimizer_min_crop_size = Minimalny rozmiar uprawnień
subsettings_video_optimizer_min_crop_size_hint = Minimum piksel do przycięcia na dowolnym boku (1-1000). Mniejsze przycięcia są ignorowane. Domyślne: 5
...
crop_videos_text = Przycinaj filmy
crop_video_confirmation_text = Czy na pewno chcesz przyciąć wybrane filmy?
crop_reencode_video_text = Ponownie zakoduj wideo
reencode_videos_text = Ponownie zakoduj wideo
