...
motto = Ten program jest darmowy w użyciu i zawsze taki pozostanie. Szczegóły znajdują się w licencji MIT/GPL.
...
build_info_build = Buduj
build_info_runtime = Czas działania
build_info_refresh = Odśwież
build_info_diagnostic_button = Informacje diagnostyczne
build_info_diagnostic_title = Informacje diagnostyczne
build_info_library_restart_hint = * Biblioteki (HEIF, LibRAW, AVIF) odzwierciedlają stan systemu w momencie uruchomienia - zrestartuj aplikację po zainstalowaniu lub usunięciu wtyczek.
build_info_test_file = Plik testowy
other_apps_title = Inne aplikacje qarmin
other_apps_open_source_note = Wszystkie wymienione aplikacje są darmowe i open source.
other_apps_open_button = Otwórz
other_apps_szyszka_desc = Szybka i potężna zmiana pliku.
other_apps_mykrut_desc = Prosty szybko i opiniowany menedżer plików Linux.
other_apps_dcmki_viewer_desc = Prosta przeglądarka DICOM.
other_apps_video_thumbnailer_desc = Wrapper wokół generatora miniatur wideo używanego w Czkawce.
other_apps_space_finder_desc = Prosta wyszukiwarka największych plików w Twoim systemie.
other_apps_system_info_collector_desc = Zbiera użycie pamięci RAM/CPU z systemu operacyjnego i pokazuje je jako wykresy.
