...
rust_hashed_images = 哈希 {$items_stats} 图像({ $size_stats })
rust_compared_image_hashes = 比較{ $items_stats}的圖像雜徵
rust_hashed_videos = 哈希 { $items_stats } 個影片
rust_created_thumbnails = 為 { $items_stats }支影片創建縮圖
...
rust_failed_to_rename_file = 失敗將檔案{ $old_path }重命名為{ $new_path }，錯誤:{ $error }
rust_no_included_directories = 未設定要掃描的資料夾，無法開始掃描。
rust_all_dirs_referenced = 所有包含的資料夾均設為參考資料夾，無法開始掃描。
rust_found_empty_folders = 找到 {$items_found} 空資料夾在{$time}
rust_found_empty_files = 找到 { $items_found } 個空文件在 { $time }
rust_found_similar_images = 找到 { $items_found } 個相似圖檔，在 { $groups } 組中，於 { $time }
rust_found_similar_videos = 找到 {$items_found} 個相似的影片檔案，在{$groups}組中，於{$time}
rust_no_similarity_method_selected = 未選擇相似度方法前，無法找到相似的音樂檔案。
rust_found_similar_music_files = 找到 {$items_found} 隨機音樂檔案，在{$groups}群組中於{$time}
rust_found_invalid_symlinks = 找到{$items_found}個無效捷徑文件在{$time}
rust_found_temporary_files = 找到 ${ $items_found } 個暫存文件在 ${ $time }
rust_no_file_type_selected = 無法找到Broken檔案，請先選定檔案類型。
rust_found_broken_files = 找到 {$items_found} 壞掉的檔案佔用了{$size}在{$time}
rust_found_bad_extensions = 找到 { $items_found } 個具有壞擴展名的檔案在 { $time }
rust_found_duplicate_files = 找到 {$items_found} licate 檔案在 {$groups} 組裡，佔用 {$size} 在 {$time}
rust_found_duplicate_files_no_lost_space = 找到{$items_found}個重複檔案在{$groups}組內於{$time}
rust_found_big_files = 找到 { $items_found } 個大文件，大小為 { $size }，在 { $time } 完成
...
rust_delete_summary = 刪除 { $deleted } 階段, 刪除失敗 { $failed } 階段, 總共 { $total } 階段
rust_rename_summary = 重命名 { $renamed } 項目，失敗重命名 { $failed } 項目，共 { $total } 項目
rust_move_summary = 移動了{$moved}項目，{$failed}項未成功移動，共{$total}个项目
...
rust_delete_confirmation_number_simple = 選取了 { $items } 項。
rust_delete_confirmation_number_groups = 選択 {$items} 項目於{$groups}組中。
rust_delete_confirmation_selected_all_in_group = 所有選項皆已從{$groups}群組中選取。
...
motto = 
        這個程式免費供使用，並且永遠會如此。
        請參閱MIT/MPL授權以取得詳細資料。
unicorn = 你可能不會去看独角兽，但 Unicorn 嚴 prez 時刻看著你。
repository = 儲存庫
instruction = 說明
donation = 贊助
translation = 翻譯
...
settings_thread_number = 線索編號
settings_restart_required = 
        ---您需要重新啟動應用程式以套用线數更改---
        注意：這裡的"線數"是專有名詞或特殊格式，根據上下文可能需要調整為更合適的中文表達。如果確切含义是指“線數”，則可以保留 untranslated。
settings_duplicate_image_preview = 圖像預覽
settings_duplicate_hide_hard_links = 隱藏硬連結
settings_duplicate_minimal_hash_cache_size = 緩存文件的最小大小 - 哈希（KB）
settings_duplicate_use_prehash = 使用預散列
settings_duplicate_minimal_prehash_cache_size = 緩存文件的最小尺寸 - 預雜哈希（KB）
settings_duplicate_delete_outdated_entries = 刪除自動過時的項目
settings_similar_images_show_image_preview = 圖像預覽
settings_similar_images_hide_hard_links = 隱藏硬連結
settings_delete_outdated_entries = 刪除自動過時的項目
...
settings_edit_name = 編輯姓名
settings_choose_name_for_prefix = 選擇名稱作為前綴
settings_save = 儲存
settings_load = 載入
settings_reset = 重設
settings_similar_videos_tool = 相似視頻工具
settings_similar_images_tool = 相似圖像工具
settings_similar_music_tool = 相似音樂工具
settings_general_settings = 一般設定
settings_settings = 設定
settings_load_tabs_sizes_at_startup = 載入_tab大小於啟動時
settings_load_windows_size_at_startup = 載入.Startupwindows大小
settings_limit_lines_of_messages = 限製訊息行數為500行（用以應對TextEdit widgets載入慢的问题）
popup_save_title = 儲存結果
popup_save_message = 這將把結果保存到3個不同的檔案中
popup_rename_title = 重新命名檔案
popup_rename_message = 這將會把選中的文件擴展名改為更合適的 Ones
...
searching = 正在搜尋...
subsettings_videos_crop_detect = 作物檢測方法
subsettings_videos_skip_forward_amount = 跳過時長 [s]
subsettings_videos_vid_hash_duration = VIDEO 哈希 持續時間
subsettings_videos_thumbnail_percentage = 視頻中縮圖位置 (%)
settings_cache_number_size_text = Cache 檔案大小: { $size }, 檔案數量: { $number }
settings_video_thumbnails_number_size_text = VIDEO 縮圖大小：{ $size }, 檔案數量：{ $number }
