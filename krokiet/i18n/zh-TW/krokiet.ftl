...
rust_checked_images = 已檢查 { $items_stats } 圖像 ({ $size_stats })
...
settings_application_scale_hint_text = 手動應用量度 (例如：1.0 = 100%)
settings_restart_required_scale_text = 您需要重新啟動應用程式以應用比例變更。
settings_use_manual_application_scale_text = 使用手動應用量表
settings_similar_videos_generate_thumbnail_grid_instead_of_single_image_hint_text = 產生一個縮圖網格而不是單張圖片 (可能需要更長的時間)
settings_similar_videos_generate_thumbnail_grid_instead_of_single_image = 生成縮圖網格而不是單張圖片
settings_similar_videos_clear_unused_thumbnails = 刪除啟動時超過 7 天未使用的影片缩圖
...
subsettings_video_optimizer_image_threshold = 圖像閾值
...
optimize_text = 優化圖片
optimize_confirmation_text = 您確定要優化所選項目嗎？
