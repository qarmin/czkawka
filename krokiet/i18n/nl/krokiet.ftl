# In Rust translations
rust_init_error_title = Kritische fout tijdens app opstarten
rust_init_error_message =
    Een kritische fout is opgetreden bij het starten van de applicatie:
    
    { $error_message }
    
    Dit kan worden veroorzaakt door ontbrekende of defecte OpenGL/Vulkan drivers, het uitvoeren van de applicatie in een virtuele machine of een bug in Krokiet of een van zijn libraries.
rust_loaded_preset = Voorinstelling geladen { $preset_idx }
rust_file_already_exists = Bestand "{ $file }" bestaat al en zal niet worden overschreven
rust_error_removing_file_after_copy = Fout bij het verwijderen van bestand "{ $file }" (na kopiëren naar een andere partitie), reden: { $reason }
rust_error_copying_file = Fout bij kopiëren "{ $input }" naar "{ $output }", reden: { $reason }
rust_failed_to_rename_file = Kan bestand { $old_path } niet hernoemen naar { $new_path }, fout: { $error }
rust_rename_single_invalid_name = Nieuwe naam mag niet leeg zijn of pad scheidingstekens bevatten.
rust_rename_single_target_exists = Er bestaat al een bestand met deze naam in deze map.
rust_rename_single_same_name = Nieuwe naam is dezelfde als de huidige.
rust_renamed_file = { $old_name } hernoemd naar { $new_name }
rust_no_included_paths = Kan geen scan starten wanneer geen ingesloten paden zijn ingesteld.
rust_all_paths_referenced = Kan geen scan starten wanneer alle opgenomen paden als verwijzen paden zijn ingesteld, u moet het checkbox-vak naast het invoerpad uitschakelen.
rust_found_empty_folders = Gevonden { $items_found } lege mappen in { $time }
rust_found_empty_files = Gevonden { $items_found } lege bestanden in { $time }
rust_found_similar_images = { $items_found } vergelijkbare afbeeldingsbestanden gevonden in { $groups } groepen in { $time }
rust_found_similar_videos = { $items_found } vergelijkbare videobestanden gevonden in { $groups } groepen in { $time }
rust_found_similar_music_files = Gevonden { $items_found } vergelijkbare muziekbestanden in { $groups } groepen in { $time }
rust_found_invalid_symlinks = Gevonden { $items_found } ongeldige symlinks in { $time }
rust_found_temporary_files = Gevonden { $items_found } tijdelijke bestanden in { $time }
rust_no_file_type_selected = Kan defecte bestanden niet vinden zonder geselecteerde bestandstype.
rust_found_broken_files = Gevonden { $items_found } defecte bestanden in { $size } in { $time }
rust_found_bad_extensions = { $items_found } bestanden met ongeldige extensies gevonden in { $time }
rust_found_bad_names = Vond { $items_found } bestanden met slechte namen in { $time }
rust_found_video_optimizer = Vond { $items_found } bestanden om te optimaliseren in { $time }
rust_found_duplicate_files = Gevonden { $items_found } dubbele bestanden in { $groups } groepen die { $size } in { $time } nemen
rust_found_duplicate_files_no_lost_space = Gevonden { $items_found } dubbele bestanden in { $groups } groepen in { $time }
rust_found_big_files = Gevonden { $items_found } grote bestanden met grootte { $size } in { $time }
rust_found_exif_files = Vond { $items_found } bestanden met exif data in { $time }
rust_cannot_load_preset = Kan vooraf ingestelde { $preset_idx } niet wijzigen en laden - reden { $reason }, met behulp van standaardinstellingen
rust_saved_preset = Opgeslagen voorinstelling { $preset_idx }
rust_cannot_save_preset = Kan voorinstelling { $preset_idx } niet opslaan - reden { $reason }
rust_reset_preset = Herstel voorinstelling { $preset_idx }
rust_cannot_create_output_folder = Kan de uitvoermap { $output_folder } niet aanmaken, reden: { $error }
rust_delete_summary = { $deleted } items verwijderd, { $failed } items kunnen niet verwijderd worden, uit { $total } items
rust_rename_summary = { $renamed } items hernoemd, kan { $failed } items niet hernoemen, uit { $total } items
rust_move_summary = { $moved } items verplaatst, { $failed } items konden niet verplaatst worden, van { $total } items
rust_hardlink_summary = Hardgelinkte { $hardlinked } items, faalde om { $failed } items te hardlinken, van { $total } items
rust_symlink_summary = Gesymlinkt { $symlinked } items, kan { $failed } items niet symlinken, van { $total } items
rust_optimize_video_summary = Geoptimaliseerd { $optimized } video's, mislukte optimalisatie van { $failed } video's, van { $total } video's
rust_clean_exif_summary = Opgeschoonde EXIF van { $cleaned } bestanden, mislukt bij het opschonen van { $failed } bestanden, van { $total } bestanden
rust_no_files_deleted = Geen bestanden of mappen geselecteerd voor verwijdering
rust_no_files_renamed = Geen bestanden of mappen geselecteerd voor hernoemen
rust_no_files_moved = Geen bestanden of mappen geselecteerd om te verplaatsen
rust_no_files_hardlinked = Geen bestanden of mappen geselecteerd voor hardlinken
rust_no_files_symlinked = Geen bestanden of mappen geselecteerd voor symbolische links
rust_no_videos_optimized = Geen video's geselecteerd voor optimalisatie
rust_no_exif_cleaned = Geen bestanden geselecteerd voor EXIF reiniging
rust_delete_confirmation = Weet u zeker dat u de geselecteerde items wilt verwijderen?
rust_delete_confirmation_number_simple = { $items } items geselecteerd.
rust_delete_confirmation_number_groups = { $items } items geselecteerd in { $groups } groepen.
rust_delete_confirmation_selected_all_in_group = Alle items geselecteerd in { $groups } groepen.
rust_trash_confirmation = Weet u zeker dat u de geselecteerde items naar de prullenbak wilt verplaatsen?
rust_trash_confirmation_unsupported_volumes = Opmerking: niet alle volumes ondersteunen het verplaatsen naar prullenbak. Voor niet-ondersteunde volumes zal deze operatie altijd mislukken.
rust_trash_confirmation_number_simple = { $items } items geselecteerd.
rust_trash_confirmation_number_groups = { $items } items geselecteerd in { $groups } groepen.
rust_trash_confirmation_selected_all_in_group = Alle items geselecteerd in { $groups } groepen.
rust_move_confirmation = Weet u zeker dat u de geselecteerde items wilt verplaatsen?
rust_move_confirmation_number_simple = { $items } items geselecteerd.
rust_clean_exif_confirmation = Weet u zeker dat u de EXIF-gegevens uit de geselecteerde items wilt verwijderen?
rust_clean_exif_confirmation_number_simple = { $items } items geselecteerd.
clean_exif_overwrite_files_text = Schrijf bestanden over
rust_optimize_video_confirmation = Weet u zeker dat u de geselecteerde video's wilt optimaliseren?
rust_optimize_video_confirmation_number_simple = { $items } items geselecteerd.
rust_hardlink_confirmation = Weet u zeker dat u hardlinks wilt aanmaken voor de geselecteerde items?
rust_hardlink_confirmation_number_simple = { $items } items geselecteerd.
rust_symlink_confirmation = Weet u zeker dat u symbolische links wilt aanmaken voor de geselecteerde items?
rust_symlink_confirmation_number_simple = { $items } items geselecteerd.
rust_rename_confirmation = Weet u zeker dat u de geselecteerde items wilt hernoemen?
rust_rename_confirmation_number_simple = { $items } items geselecteerd.
rust_cache_processed_files = Verwerkt { $files } cache bestanden
rust_cache_entries_stats = Verwijderd { $removed } entries uit alle { $all }, { $left } overgebleven
rust_cache_size_reduced = Verkleinde cachebestandsgrootte met { $size }
rust_cache_time_elapsed = Tijd verstreken: { $time }
rust_symlink_failed = symlink { $name } naar { $target }mislukt, reden { $reason }
rust_hardlink_failed = Faalde bij het hardlinken van { $name } naar { $target }, reden { $reason }

# Slint translations, but in arrays

column_selection = Selectie
column_size = Grootte
column_file_name = Bestandsnaam
column_path = Pad
column_modification_date = Wijziging datum
column_similarity = Vergelijkbaarheid
column_dimensions = Afmetingen
column_new_dimensions = Nieuwe Dimensies
column_title = Titel
column_artist = Kunstenaar
column_year = Jaar
column_bitrate = Bitsnelheid
column_length = Lengte
column_genre = Genre
column_type_of_error = Type fout
column_symlink_name = Symlink Naam
column_symlink_folder = Symlink map
column_destination_path = Bestemmingspad
column_current_extension = Huidige extensie
column_proper_extension = Juiste extensie
column_fps = FPS
column_codec = Codec
column_duration = Duur
column_exif_tags = EXIF tags
column_new_name = Nieuwe Naam
column_full_path = Volledig pad
# Slint translations
ok_button = Ok
cancel_button = Annuleren
yes_text = ja
no_text = nee
do_you_want_to_continue = Wilt u doorgaan?
main_window_title = Krokiet (Kroket) - Gegevensopschoner
file_dialog_open = Sluit het dialoogvenster om door te gaan
scan_button = Scannen
stop_button = Stoppen
stop_text = Stoppen
select_button = Selecteren
move_button = Verplaatsen
delete_button = Verwijderen
trash_button = Prullenbak
save_button = Opslaan
sort_button = Sorteren
rename_button = Hernoemen
motto = Dit programma is gratis te gebruiken en dat blijft zo. Zie de MIT/GPL-licentie voor de details.
unicorn = U mag niet naar een eenhoorn kijken, maar de eenhoorn kijkt altijd naar u.
repository = Bewaarplaats
instruction = Instructie
donation = Donatie
translation = Vertaling
other_apps = Andere apps
build_info_build = Bouw
build_info_runtime = Runtime
build_info_refresh = Vernieuwen
build_info_diagnostic_button = Diagnostische informatie
build_info_diagnostic_title = Diagnostische informatie
build_info_library_restart_hint = * De bibliotheekonderzoeken (HEIF, LibRAW, AVIF) geven de status weer bij het opstarten - start de app opnieuw na het installeren of verwijderen van plug-ins.
build_info_test_file = Testbestand
other_apps_title = Andere toepassingen door qarmin
other_apps_open_source_note = Alle getoonde toepassingen zijn gratis en open source.
other_apps_open_button = Open
other_apps_szyszka_desc = Snelle en krachtige bestandshernoemer.
other_apps_mykrut_desc = Eenvoudige, snelle en eigenzinnige Linux bestandsbeheerder.
other_apps_dcmki_viewer_desc = Eenvoudige DICOM kijker.
other_apps_video_thumbnailer_desc = Wrapper om de video-thumbnail generator die gebruikt wordt in Czkawka.
other_apps_space_finder_desc = Eenvoudige zoeker van de grootste bestanden op uw systeem.
other_apps_system_info_collector_desc = Verzamelt RAM/CPU-gebruik van het besturingssysteem en toont het als grafiek.
included_paths = Ingesloten paden
excluded_paths = Uitgesloten Paden
ref = Ref:
path = Pad
tool_duplicate_files = Dubbele bestanden
tool_empty_folders = Lege mappen
tool_big_files = Grote bestanden
tool_empty_files = Lege bestanden
tool_temporary_files = Tijdelijke bestanden
tool_similar_images = Vergelijkbare afbeeldingen
tool_similar_videos = Vergelijkbare video's
tool_music_duplicates = Muziek duplicaten
tool_invalid_symlinks = Ongeldige Symlinks
tool_broken_files = Kapotte Bestanden
tool_bad_extensions = Slechte extensies
tool_bad_names = Slechte Namen
tool_video_optimizer = Video optimalisatie
tool_exif_remover = Exif Verwijderaar
sort_by_full_name = Sorteren op volledige naam
sort_by_focus = Sorteren op focus
sort_reverse = Omgekeerde volgorde
selection_all = Alles selecteren
selection_deselect_all = Deselecteer alles
selection_invert_selection = Selectie omkeren
selection_invert_selection_in_group = Selectie in groep omkeren
selection_the_biggest_size = Selecteer de grootste grootte
selection_the_biggest_resolution = Selecteer de grootste resolutie
selection_the_smallest_size = Selecteer de kleinste grootte
selection_the_smallest_resolution = Selecteer de kleinste resolutie
selection_newest = Selecteer nieuwste
selection_oldest = Selecteer oudste
selection_shortest_path = Selecteer het kortste pad
selection_longest_path = Selecteer het langste pad
selection_all_except_biggest_size = Selecteer alle behalve de grootste grootte
selection_all_except_smallest_size = Selecteer alle behalve kleine grootte
selection_all_except_biggest_resolution = Alles selecteren behalve de grootste resolutie
selection_all_except_smallest_resolution = Selecteer alle behalve kleine resolutie
selection_all_except_newest = Selecteer alles behalve nieuwste
selection_all_except_oldest = Alles selecteren behalve oudste
selection_all_except_longest_path = Selecteer alles behalve het langste pad
selection_all_except_shortest_path = Selecteer alles behalve het kortste pad
selection_custom_select_unselect = Aangepaste Selectie/Deselecteer
stage_current = Huidige fase:
stage_all = Alle fasen:
subsettings = Subinstellingen
subsettings_images_hash_size = Hash grootte
subsettings_images_resize_algorithm = Algoritme verkleinen
subsettings_images_ignore_same_size = Negeer afbeeldingen met dezelfde grootte
subsettings_images_ignore_same_resolution = Negeer afbeeldingen met dezelfde resolutie
subsettings_images_max_difference = Max verschil
subsettings_images_geometric_invariance = Geometrische invariantie
subsettings_images_duplicates_hash_type = Hash-Type
subsettings_duplicates_check_method = Controleer methode
subsettings_duplicates_name_case_sensitive = Hoofdlettergevoelig (alleen naammodi)
subsettings_biggest_files_sub_method = Methode
subsettings_biggest_files_sub_number_of_files = Aantal bestanden
option_search_mode_biggest = De Grootste
option_search_mode_smallest = De Kleinste
option_music_method_tags = Labels
option_music_method_fingerprint = Vingerafdruk
option_check_method_hash = Hash
option_check_method_size = Grootte
option_check_method_name = Naam
option_check_method_size_and_name = Grootte en naam
option_crop_detect_letterbox = LetterBox
option_crop_detect_motion = Beweging
option_crop_detect_none = Geen
option_video_crop_type_black_bars = Zwarte staven
option_video_crop_type_static_content = Statische inhoud
option_video_optimizer_mode_crop = Bijsnijden
option_video_optimizer_mode_transcode = Transcodeer
option_noise_reduction_none = Geen
option_noise_reduction_hqdn3d = hqdn3d (snel)
subsettings_videos_max_difference = Max verschil
subsettings_videos_ignore_same_size = Negeer video's met dezelfde grootte
subsettings_videos_ignore_same_resolution = Negeer video's met dezelfde resolutie
subsettings_videos_audio_check_content = Vergelijk met vingerafdruk
subsettings_videos_audio_preset = Snelle instelling
subsettings_videos_audio_preset_custom = Aangepaste
subsettings_videos_audio_preset_identical = Identische video's
subsettings_videos_audio_preset_clip = Clip in langere video
subsettings_videos_audio_preset_similar = Vergelijkbare inhoud
subsettings_videos_visual_preset = Snelle instelling
subsettings_videos_visual_preset_custom = Aangepaste
subsettings_videos_visual_preset_near_identical = Bijna identiek
subsettings_videos_visual_preset_similar = Vergelijkbaar
subsettings_videos_visual_preset_movies = Films (lange skip)
subsettings_videos_duration_tolerance_pct = Duur tolerantie [%]
subsettings_videos_min_matching_windows = Min overeenkomende vensters
subsettings_videos_subclip_min_match = Min. overeenkomst subclip
subsettings_videos_audio_similarity_percent = Vergelijkbaarheid [%]
subsettings_videos_audio_length_ratio = Min. lengte-verhouding (korter/langer)
subsettings_videos_audio_min_duration_seconds = Minimale bestandsduur [s]
subsettings_videos_audio_maximum_difference = Max audio verschil
subsettings_music_audio_check_type = Type audio-controle
subsettings_music_approximate_comparison = Geschatte Tag vergelijking
subsettings_music_compared_tags = Vergeleken labels
subsettings_music_title = Titel
subsettings_music_artist = Kunstenaar
subsettings_music_bitrate = Bitsnelheid
subsettings_music_genre = Genre
subsettings_music_year = Jaar
subsettings_music_length = Lengte
subsettings_music_max_difference = Max verschil
subsettings_music_minimal_fragment_duration = Minimale fragment duur
subsettings_music_compare_fingerprints_only_with_similar_titles = Vergelijk binnen groepen van vergelijkbare titels
subsettings_broken_files_type = Type bestanden om te controleren
subsettings_empty_files_type = Extra bestandstypes om te vinden
subsettings_empty_files_zero_byte_content = Bestanden gevuld met null bytes
subsettings_empty_files_zero_byte_content_hint = Vind ook niet-lege bestanden waarvan de gehele inhoud uit null bytes (0x00) bestaat.
subsettings_empty_files_non_printable_content = Bestanden gevuld met niet-afdrukbare tekens
subsettings_empty_files_non_printable_content_hint = Vindt ook niet-lege bestanden die bestaan uit alleen niet-afdrukbare ASCII-tekens: nul, ruimte, tab, CR, LF, VT, FF.
subsettings_broken_files_audio = Geluid
subsettings_broken_files_pdf = PDF
subsettings_broken_files_archive = Archief
subsettings_broken_files_image = Afbeelding
subsettings_broken_files_video_ffprobe = Video (ffprobe)
subsettings_broken_files_video_ffprobe_info = Quick video controle met behulp van ffprobe (header validatie).
subsettings_broken_files_video_ffmpeg = Video (ffmpeg)
subsettings_broken_files_video_ffmpeg_info = Diepe video-controle met behulp van ffmpeg (volledige decode). Erg langzaam en kan pedantische fouten detecteren, zelfs als het bestand goed afspeelt.
subsettings_broken_files_font = Lettertype
subsettings_broken_files_markup = Markup (JSON/XML/TOML)
subsettings_bad_names_issues = Bestandsnaamcontroles
subsettings_bad_names_uppercase_extension = Extensie met hoofdletters
subsettings_bad_names_uppercase_extension_hint = Vindt bestanden met hoofdletters in de extensie (bijv. .JPG, .Mp3) en suggereert de kleine letters versie
subsettings_bad_names_emoji_used = Emoji in naam
subsettings_bad_names_emoji_used_hint = Vindt bestanden met emoji-tekens (😀, 🎉, enz.) in de naam en suggereert ze te verwijderen
subsettings_bad_names_space_at_start_end = Spaties aan begin/einde
subsettings_bad_names_space_at_start_end_hint = Vindt bestanden met spaties aan het begin of aan het einde van de naam en suggereert deze te trimmen
subsettings_bad_names_non_ascii = Niet-ASCII tekens
subsettings_bad_names_non_ascii_hint = Vindt niet-ASCII tekens (ą, ć, ñ, enz.) en suggereert ze te vervangen door ASCII equivalenten (a, c, n) of verwijderen indien er geen mapping bestaat
subsettings_bad_names_restricted_charset = Beperkte tekencode
subsettings_bad_names_restricted_charset_hint = Translitereert niet-ASCII tekens naar ASCII, en vindt vervolgens bestanden die tekens buiten 0-9a-zA-Z en de door de gebruiker toegestane tekens bevatten
subsettings_bad_names_allowed_chars = Toegestane tekens
subsettings_bad_names_remove_duplicated = Verdubbelde tekens
subsettings_bad_names_remove_duplicated_hint = Vindt opeenvolgende verdubbelde niet-alfanumerieke tekens (bijv. "file---name..txt") en suggereert het verwijderen van duplicaten
settings_global_settings = Globale instellingen
settings_dark_theme = Donker thema
settings_show_only_icons = Alleen pictogrammen weergeven
settings_excluded_items = Uitgesloten item:
settings_allowed_extensions = Toegestane extensies:
settings_excluded_extensions = Uitgesloten extensies:
settings_file_size = Bestandsomvang (Kilobaten)
settings_minimum_file_size = Min.:
settings_maximum_file_size = Max:
settings_recursive_search = Recursieve zoekopdracht
settings_use_cache = Gebruik cache
settings_save_as_json = Sla ook de cache op als JSON-bestand
settings_ignore_other_filesystems = Negeer andere bestandssystemen (alleen Linux)
settings_delete_outdated_cache_entries = Verwijder automatisch verouderde cache-entries
settings_delete_outdated_cache_entries_hint = Wanneer ingeschakeld, zal de app tijdens het laden van de cache (maximaal één keer per week) controleren of de opgeslagen records nog steeds naar bestaande en ongewijzigde bestanden/gegevens wijzen
settings_hide_hard_links = Verberg hardlinks
settings_hide_hard_links_hint = Verberg hardlinks naar dezelfde bestanden in de resultaten
settings_thread_number = Thread nummer
settings_restart_required = ---U moet de app herstarten om wijzigingen in thread number--- toe te passen
settings_duplicate_image_preview = Voorvertoning afbeelding
settings_duplicate_minimal_hash_cache_size = Minimale grootte van gecachete bestanden - Hash (KB)
settings_duplicate_use_prehash = Gebruik prehash
settings_duplicate_minimal_prehash_cache_size = Minimale grootte van gecachete bestanden - Prehash (KB)
settings_similar_images_show_image_preview = Voorvertoning afbeelding
settings_similar_videos_preview_hint = Voorbeeld is alleen zichtbaar wanneer "Miniaturen genereren" is ingeschakeld of wanneer een miniatuur al is gegenereerd.
settings_application_scale_text = Toepassingsschaal
settings_application_scale_hint_text = Wanneer handmatige schaal is ingeschakeld, staat dit toe dat u een aangepaste schaalfactor kiest, maar schakelt het automatisch schalen op basis van de DPI van het scherm volledig uit.
settings_restart_required_scale_text = ---U moet de app opnieuw starten om wijzigingen in de schaal toe te passen---
settings_use_manual_application_scale_text = Gebruik handmatige toepassingsschaal
settings_video_thumbnails_preview = Voorbeeldweergave
settings_open_config_folder = Open configuratiemap
settings_open_cache_folder = Open cachemap
settings_language = Taal
settings_current_preset = Huidige voorkeursinstelling:
settings_edit_name = Naam bewerken
settings_choose_name_for_prefix = Kies een naam voor de prefix
settings_save = Opslaan
settings_load = Laden
settings_reset = Herstel
settings_similar_videos_tool = Vergelijkbare video's gereedschap
settings_video_thumbnails_clear_unused_thumbnails = Verwijder ongebruikte videobeschrijvingen ouder dan 7 dagen bij het opstarten van de app
settings_video_thumbnails_header = Video Miniatuurfoto's
settings_video_thumbnails_generate = Genereer miniaturen
settings_video_thumbnails_position = Thumbnail positie in video (%)
settings_video_thumbnails_generate_grid = Maak een thumbnail-raster in plaats van één afbeelding te genereren
settings_video_thumbnails_generate_grid_hint = Het genereren van meerdere afbeeldingen in een raster is veel langzamer dan het genereren van een enkele miniatuur
settings_video_thumbnails_grid_tiles_per_side = Aantal tegels per zijde in de miniaturen raster
settings_video_thumbnails_grid_tiles_per_side_hint = Aantal miniaturen tiles per kant in het raster. Bijvoorbeeld, het selecteren van 2 creëert een 2 x 2 raster, wat resulteert in een enkele miniatuur bestaande uit 4 afbeeldingen.
settings_similar_images_tool = Vergelijkbare afbeeldingen gereedschap
settings_general_settings = Algemene instellingen
settings_select_header_text = Popup opties selecteren
settings_select_group_size_text = Grootte
settings_select_group_date_text = Wijziging datum
settings_select_group_resolution_text = Resolutie
settings_select_group_path_text = Pad
settings_select_label_one_smallest_text = Een kleinste
settings_select_label_one_biggest_text = Één grootste
settings_select_label_except_smallest_text = Behalve kleinste
settings_select_label_except_biggest_text = Behalve de grootste
settings_select_label_one_oldest_text = Één oudste
settings_select_label_one_newest_text = Eén nieuwste
settings_select_label_except_oldest_text = Behalve oudste
settings_select_label_except_newest_text = Behalve nieuwste
settings_select_label_shortest_text = Kortste
settings_select_label_longest_text = Langste
settings_select_label_except_shortest_text = Behalve kortste
settings_select_label_except_longest_text = Behalve de langste
settings_cache_header_text = Cache Instellingen
settings_clean_cache_button_text = Verouderde cache opschonen
settings_settings = Instellingen
settings_load_tabs_sizes_at_startup = Laad tabbladen grootte bij het opstarten
settings_load_windows_size_at_startup = Laad venstergrootte bij het opstarten
settings_limit_lines_of_messages = Berichten beperken tot 500 regels(workaround voor langzame TextEdit widget)
settings_play_audio_on_scan_completion_text = Speel geluid wanneer scan succesvol is voltooid
settings_audio_feature_hint_text = Alleen beschikbaar bij het compileren met de audio-functie
settings_audio_env_variable_hint_text = Kan worden veranderd, door de KROKIET_AUDIO_STOP_FILE omgevingvariabele in te stellen op een geldige audiobestandspad
settings_show_notification_on_scan_completion_text = Melding weergeven als scan voltooid is
settings_notification_feature_hint_text = Alleen beschikbaar bij compileren van notificaties
settings_notification_focused_hint_text = Kennisgeving wordt mogelijk niet weergegeven als een pop-up wanneer het app-venster gefocust is
popup_save_title = Resultaten opslaan
popup_save_message = Dit zal resultaten opslaan naar 3 verschillende bestanden
popup_rename_title = Bestanden hernoemen
popup_rename_single_title = Hernoem bestand
popup_rename_single_same_name = Nieuwe naam is dezelfde als de huidige.
popup_rename_single_target_exists = Er bestaat al een bestand met deze naam in deze map.
popup_error_title = Foutmelding
popup_new_paths_title = Voeg de paden toe, één per regel
popup_move_title = Bestanden verplaatsen
popup_move_copy_checkbox = Bestanden kopiëren in plaats van verplaatsen
popup_move_preserve_folder_checkbox = Mapstructuur behouden
popup_move_rename_on_conflict_checkbox = Hernoem bij conflict (in plaats van overslaan)
move_confirmation_text = Weet u zeker dat u de geselecteerde items wilt verplaatsen?
rename_confirmation_text = Weet u zeker dat u de geselecteerde items wilt hernoemen?
delete = Items verwijderen
trash = Items verplaatsen naar prullenbak
stopping_scan = Scannen stoppen, even geduld...
searching = Zoeken...
subsettings_videos_crop_detect = Letterbox crop detecteren
subsettings_videos_skip_forward_amount = Overslaan duur [s]
subsettings_videos_vid_hash_duration = Video hash duur
subsettings_videos_window_count = Aantal vensters
settings_cache_number_size_text = Cache bestanden grootte: { $size }, aantal bestanden: { $number }
settings_video_thumbnails_number_size_text = Video miniatuurgrootte: { $size }, aantal bestanden: { $number }
settings_log_number_size_text = Log bestanden grootte: { $size }, aantal bestanden: { $number }
popup_clean_cache_title_text = Schoon Cache Op
popup_clean_cache_confirmation_text = Weet u zeker dat u verouderde cache-items wilt opschonen? Dit verwijdert cache-items voor bestanden die niet meer bestaan of zijn gewijzigd.
popup_clean_cache_progress_text = Verwerking cachebestand:
popup_clean_cache_current_file_text = Huidige bestand:
popup_clean_cache_file_progress_text = Huidige bestandsgang:
popup_clean_cache_overall_progress_text = Algemeen vooruitgangsrapport:
popup_clean_cache_stopped_by_user_text = Cache opschonen is gestopt door gebruiker
popup_clean_cache_finished_text = Cache opschonen voltooid met succes!
popup_clean_cache_error_details_text = Foutdetails:
popup_clean_cache_files_with_errors = Bestanden met fouten:
subsettings_video_optimizer_mode = Modus
subsettings_video_optimizer_crop_type = Bijsnijdtype
subsettings_video_optimizer_black_pixel_threshold = Zwarte Pixel Drempel
subsettings_video_optimizer_black_pixel_threshold_hint = Maxima RGB-waarde voor elke kleurkanal om als zwart (0-128) te worden beschouwd. Standaard: 20
subsettings_video_optimizer_black_bar_min_percentage = Zwarte Balk Min Percentage
subsettings_video_optimizer_black_bar_min_percentage_hint = Minimum percentage van zwarte pixels in een rij/kolom om als een zwarte balk te worden beschouwd (50-100). Standaard: 90
subsettings_video_optimizer_max_samples = Maximaal aantal samples
subsettings_video_optimizer_max_samples_hint = Maxima aantal frames te analyseren per video (5-1000). Standaard: 60
subsettings_video_optimizer_min_crop_size = Min. bijsnijdgrootte
subsettings_video_optimizer_min_crop_size_hint = Minimale pixels om bij te snijden aan elke zijde (1-1000). Kleinere bijsnijdingen worden genegeerd. Standaard: 5
subsettings_video_optimizer_video_codec = Videocodec
subsettings_video_optimizer_excluded_codecs = Uitgesloten codecs
subsettings_video_optimizer_video_quality = Video kwaliteit (CRF)
subsettings_video_optimizer_noise_reduction = Ruis Reductie
subsettings_video_optimizer_noise_reduction_strength = NR Kracht
subsettings_video_optimizer_use_custom_command = Gebruik aangepaste FFmpeg opdracht
subsettings_video_optimizer_custom_command_hint = Commando met { "{PATH}" } voor invoerbestand. Uitvoerpad wordt automatisch toegevoegd.
subsettings_video_optimizer_hardware_encoder = Hardware-encoder
subsettings_video_optimizer_generate_template = Sjabloon genereren
subsettings_reset = Herstel
subsettings_exif_ignored_tags_text = Verwaarloosde tags:
subsettings_exif_ignored_tags_hint_text = Comma-gescheiden lijst van tags om uit te sluiten van het scannen (bijv. GPS, Thumbnail). Sommige tags, zoals ImageWidth in TIFF-bestanden, zijn verborgen om te voorkomen dat het beeld wordt verbroken.
subsettings_temporary_files_extensions_text = Extensies:
subsettings_temporary_files_extensions_hint_text = Kommagescheiden lijst met als tijdelijk behandelde extensies/achtervoegsels (bv. .tmp,.bak,~). Herstelt de ingebouwde standaardwaarden.
clean_button_text = Opschonen
clean_text = Schoon EXIF data
clean_confirmation_text = Weet u zeker dat u de EXIF-gegevens uit de geselecteerde items wilt verwijderen?
crop_videos_text = Video's bijsnijden
crop_video_confirmation_text = Weet u zeker dat u de geselecteerde video's wilt bijsnijden?
crop_reencode_video_text = Video opnieuw coderen
reencode_videos_text = Video's opnieuw coderen
optimize_button_text = Optimaliseren
optimize_confirmation_text = Weet u zeker dat u de geselecteerde video's opnieuw wilt coderen?
optimize_fail_if_bigger_text = Faal als de geoptimaliseerde bestandsgrootte groter is
optimize_overwrite_files_text = Schrijf bestanden over
optimize_limit_video_size_text = Beperk videogrootte
optimize_max_width_text = Max breedte:
optimize_max_height_text = Maximale hoogte:
optimize_crf_hint = Lagere waarde = betere kwaliteit. 0 is bijna verlies, 51 is het ergste. Aanbevolen: 18-28.
optimize_noise_reduction_hint = Geluidsreductie kan de coderingstijd aanzienlijk verhogen.
optimize_noise_reduction_strength_hint = 1 = minimaal, 10 = maximale geluidsvermindering.
hardlink_button_text = Hardlink
hardlink_text = Maak hardlinks
hardlink_confirmation_text = Weet u zeker dat u hardlinks wilt aanmaken voor de geselecteerde items?
softlink_button_text = Softlink
softlink_text = Maak softlinks
softlink_confirmation_text = Weet u zeker dat u softlinks (symlinks) wilt maken voor de geselecteerde items?
popup_custom_select_title_text = Aangepaste selectie / deselecteren
popup_custom_select_button_text = Selecteren
popup_custom_unselect_button_text = Deselecteren
popup_custom_column_name_header_text = Kolom
popup_custom_filter_value_header_text = Filterwaarde (jokerteken / regex)
popup_custom_case_sensitive_text = Hoofdlettergevoelig
popup_custom_leave_one_in_group_text = Selecteer alle items behalve één, in elke groep
popup_custom_hint_str_text = Tekstkolommen: wildcards *name* /home/* *.rs
popup_custom_hint_int_text = Grootte [KB] / numerieke kolommen: >= 2048 < 512 = 0 (operatoren: >= <= > < =)
popup_custom_hint_date_text = Datumkolommen: DD-MM-YYYY of YYYY-MM-DD, optionele tijd HH:MM:SS bijv. >= 2020-01-01 of < 31-12-2022 23:59:59
popup_custom_save_restore_text = Opslaan en herstellen van filtergegevens
compare_button_text = Vergelijk
compare_back_text = Terug
compare_prev_group_text = Vorige Groep
compare_next_group_text = Volgende groep
compare_swap_text = Wisselen
compare_info_text = Informatie
compare_diff_text = Verschil
compare_split_text = Opsplitsen
compare_overlay_text = Overlay
compare_loading_title_text = Een ogenblik geduld...
compare_cancel_text = Annuleren
compare_resolution_mismatch_text = Verschillende resoluties - diff resultaten kunnen onjuist zijn
compare_computing_diff_text = Verschil berekenen...
compare_info_file_name_text = Bestandsnaam
compare_info_path_text = Pad
compare_info_size_text = Grootte
compare_info_resolution_text = Resolutie
compare_info_modification_date_text = Wijziging datum
compare_loading_images_text = Afbeeldingen laden...
compare_cancelling_text = Annuleren, even geduld...
context_menu_open_item_text = Item openen
context_menu_open_parent_text = Bovenliggende map openen
context_menu_remove_from_results_text = Verwijderen uit resultaten
context_menu_remove_all_from_folder_text = Alles uit map verwijderen
context_menu_remove_all_from_folder_recursive_text = Alles uit map verwijderen (recursief)
context_menu_select_all_from_folder_text = Alles selecteren in map
context_menu_select_all_from_folder_recursive_text = Alles selecteren uit map (recursief)
context_menu_exclude_parent_folder_text = Bovenliggende map uitsluiten
context_menu_exclude_item_text = Item uitsluiten
context_menu_copy_file_name_text = Bestandsnaam kopiëren
context_menu_copy_parent_folder_path_text = Bovenliggende map kopiëren
context_menu_copy_full_path_text = Volledig pad kopiëren
context_menu_rename_text = Hernoemen
