...
rust_cannot_load_preset = 无法加载预设 { $preset_idx }, 原因: { $reason }
rust_error_moving_to_trash = 移动到回收站时出错： { $error }
rust_error_removing_file = 删除文件时出错： { $error }
rust_delete_summary = 已删除 { $deleted } 项目，未能删除 { $failed } 项目
rust_error_creating_folder = 创建文件夹时出错： { $error }
...
rust_hashed_images = 散列的 { $items_stats } 图像 ({ $size_stats })
rust_compared_image_hashes = 比较 { $items_stats } 图像哈希
...
rust_found_duplicate_files = 找到 { $items_found } 类似的重复文件
...
subsettings_images_ignore_same_size = 忽略相同的大小
...
subsettings_videos_ignore_same_size = 忽略相同的大小
...
subsettings_music_compare_fingerprints_only_with_similar_titles = 仅与相似标题比较
...
delete = 删除项目
delete_confirmation = 您确定要删除选定的项目吗？
