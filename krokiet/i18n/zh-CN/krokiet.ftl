...
rust_found_similar_videos = 找到 { $items_found } 类似的视频文件
rust_no_similarity_method_selected = 找不到类似的音乐文件，没有选择任何相似的方法。
rust_found_similar_music_files = 找到 { $items_found } 类似的音乐文件
...
rust_found_bad_extensions = 找到 { $items_found } 文件，扩展错误
rust_found_duplicate_files = 找到 { $items_found } 重复文件
...
rust_delete_summary = 删除了 { $deleted } 项，未能从所有 { $failed } 项中删除 { $total } 项
rust_rename_summary = 已重命名的 { $renamed } 项, 未能重命名从 { $failed } 项, 从所有 { $total } 项
rust_move_summary = 移动了 { $moved } 项，不能从所有 { $failed } 项中移动 { $total } 项
...
rust_delete_confirmation = 您确定要选定的项目吗？
