...
rust_checked_images = 已检查 { $items_stats } 张图片 ({ $size_stats })
...
settings_application_scale_hint_text = 手动应用刻度 (例如，1.0 = 100%)
settings_restart_required_scale_text = 您需要重启应用以应用缩放更改
settings_use_manual_application_scale_text = 使用手动应用刻度
settings_similar_videos_generate_thumbnail_grid_instead_of_single_image_hint_text = 生成一个缩图网格而不是单个图像（可能需要更长时间）
settings_similar_videos_generate_thumbnail_grid_instead_of_single_image = 生成缩图网格而不是单个图像
settings_similar_videos_clear_unused_thumbnails = 删除启动时超过7天未使用的视频缩略图
...
subsettings_video_optimizer_image_threshold = 图像阈值
...
optimize_text = 优化图片
optimize_confirmation_text = 您确定要优化所选项目吗？
