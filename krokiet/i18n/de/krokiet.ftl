...
rust_cannot_load_preset = Kann Preset { $preset_idx }nicht laden, Grund: { $reason }
...
rust_delete_summary = Gelöschte { $deleted } Elemente, Fehler beim Entfernen von { $failed } Elementen
rust_error_creating_folder = Fehler beim Erstellen des Ordners: { $error }
...
rust_hashed_images = Hashed von { $items_stats } Bild ({ $size_stats })
rust_compared_image_hashes = Verglichen mit { $items_stats } Bild-Hash
rust_hashed_videos = Hashed von { $items_stats } Video
...
rust_found_duplicate_files = Gefunden { $items_found } ähnliche doppelte Dateien
...
manual_add_button = Manuell hinzufügen
...
sort_by_parent_name = Nach Elternnamen sortieren
...
sort_reverse = Umgekehrte Sortierung
...
subsettings_images_ignore_same_size = Gleiche Größe ignorieren
...
subsettings_videos_ignore_same_size = Gleiche Größe ignorieren
...
subsettings_music_compare_fingerprints_only_with_similar_titles = Nur mit ähnlichen Titeln vergleichen
...
delete_confirmation = Sind Sie sicher, dass Sie die ausgewählten Elemente löschen möchten?
