# In Rust translations
rust_init_error_title = خطأ حرج أثناء بدء التطبيق
rust_init_error_message =
    حدث خطأ حرج أثناء بدء التطبيق:
    
    { $error_message }
    
    قد يكون ذلك بسبب نقص أو تلف تعريفات OpenGL/Vulkan، أو تشغيل التطبيق في جهاز افتراضي أو خلل في Krokiet أو أحد مكتباته.
    
    يمكنك محاولة تشغيل إصدارات مختلفة (skia_opengl، skia_vulkan، femtovg_opengl - الافتراضي) أو مع مُسرِّع رسومي للبرامج لتحديد ما إذا كان ذلك يحل المشكلة.
rust_loaded_preset = تم تحميل الإعداد المسبق { $preset_idx }
rust_file_already_exists = الملف "{ $file }" موجود بالفعل، ولن يتم الكتابة فوقه
rust_error_removing_file_after_copy = خطأ أثناء حذف الملف "{ $file }" (بعد نسخه إلى قسم مختلف)، السبب: { $reason }
rust_error_copying_file = خطأ أثناء نسخ "{ $input }" إلى "{ $output }"، السبب: { $reason }
rust_failed_to_rename_file = فشل في إعادة تسمية الملف { $old_path } إلى { $new_path }، الخطأ: { $error }
rust_rename_single_invalid_name = الاسم الجديد يجب أن لا يكون فارغاً أو يحتوي على فواصل المسارات.
rust_rename_single_target_exists = يوجد بالفعل ملف بهذا الاسم في هذا المجلد.
rust_rename_single_same_name = الاسم الجديد هو نفس الاسم الحالي.
rust_renamed_file = تمت إعادة تسمية { $old_name } إلى { $new_name }
rust_no_included_paths = لا يمكن بدء المسح عند عدم تحديد المسارات المضمنة.
rust_all_paths_referenced = لا يمكن بدء المسح عندما تكون جميع المسارات المضمنة مضبوطة كمسارات مرجعية، تحتاج إلى تعطيل مربع الاختيار بجوار المسار المدخل.
rust_found_empty_folders = تم العثور على { $items_found } مجلدات فارغة في { $time }
rust_found_empty_files = تم العثور على { $items_found } ملفات فارغة في { $time }
rust_found_similar_images = تم العثور على { $items_found } ملفات صور مماثلة في { $groups } مجموعات في { $time }
rust_found_similar_videos = تم العثور على { $items_found } ملفات فيديو مماثلة في { $groups } مجموعات في { $time }
rust_found_similar_music_files = تم العثور على { $items_found } ملفات موسيقية مماثلة في { $groups } مجموعات في { $time }
rust_found_invalid_symlinks = تم العثور على { $items_found } روابط رمزية غير صالحة في { $time }
rust_found_temporary_files = تم العثور على { $items_found } ملفات مؤقتة في { $time }
rust_no_file_type_selected = لا يمكن العثور على الملفات المكسورة بدون أي نوع من الملفات المحددة.
rust_found_broken_files = تم العثور على { $items_found } ملفات مكسورة أخذت { $size } في { $time }
rust_found_bad_extensions = تم العثور على { $items_found } ملفات ذات ملحقات سيئة في { $time }
rust_found_bad_names = تم العثور على { $items_found } ملفات بأسماء سيئة في { $time }
rust_found_video_optimizer = تم العثور على { $items_found } ملفات لتحسينها في { $time }
rust_found_duplicate_files = تم العثور على { $items_found } ملفات مكررة في { $groups } مجموعات أخذت { $size } في { $time }
rust_found_duplicate_files_no_lost_space = تم العثور على { $items_found } ملفات مكررة في { $groups } مجموعات في { $time }
rust_found_big_files = تم العثور على { $items_found } ملفات كبيرة بحجم { $size } في { $time }
rust_found_exif_files = تم العثور على { $items_found } ملفات مع بيانات EXIF في { $time }
rust_cannot_load_preset = لا يمكن تغيير وتحميل الإعداد المسبق { $preset_idx } - السبب { $reason }، باستخدام الإعدادات الافتراضية بدلاً من ذلك
rust_saved_preset = تم حفظ الإعداد المسبق { $preset_idx }
rust_cannot_save_preset = لا يمكن حفظ الإعداد المسبق { $preset_idx } - السبب { $reason }
rust_reset_preset = إعادة تعيين الإعداد المسبق { $preset_idx }
rust_cannot_create_output_folder = لا يمكن إنشاء مجلد الإخراج { $output_folder }، السبب: { $error }
rust_delete_summary = حذف { $deleted } عناصر ، فشل في إزالة { $failed } عناصر ، من أصل { $total } عناصر
rust_rename_summary = إعادة تسمية العناصر { $renamed } ، فشل في إعادة تسمية العناصر { $failed } ، من أصل { $total } عناصر
rust_move_summary = تم نقل { $moved } عناصر، وفشل نقل { $failed } عناصر، من أصل { $total } عناصر
rust_hardlink_summary = تم إنشاء رابط صلب لـ { $hardlinked } عناصر، وفشل إنشاؤه لـ { $failed } عناصر، من أصل { $total } عناصر
rust_symlink_summary = ربط رمزي { $symlinked } عناصر، فشل ربط رمزي { $failed } عناصر، من أصل { $total } عناصر
rust_optimize_video_summary = تم تحسين { $optimized } مقاطع فيديو، وفشل تحسين { $failed }، من أصل { $total } مقاطع فيديو
rust_clean_exif_summary = تم تنظيف بيانات EXIF من { $cleaned } ملفات، وفشل تنظيفها في { $failed } ملفات، من أصل { $total } ملفات
rust_no_files_deleted = لا توجد ملفات أو مجلدات محددة للحذف
rust_no_files_renamed = لا توجد ملفات أو مجلدات محددة لإعادة التسمية
rust_no_files_moved = لا توجد ملفات أو مجلدات محددة للانتقال
rust_no_files_hardlinked = لا توجد ملفات أو مجلدات محددة لإنشاء الروابط الصلبة
rust_no_files_symlinked = لا توجد ملفات أو مجلدات محددة لإنشاء الروابط الرمزية
rust_no_videos_optimized = لا توجد فيديوهات مُحدَّدة للتحسين
rust_no_exif_cleaned = لا توجد ملفات مُحدَّدة لتنظيف EXIF
rust_delete_confirmation = هل أنت متأكد من أنك تريد حذف العناصر المحددة؟
rust_delete_confirmation_number_simple = { $items } العناصر المحددة.
rust_delete_confirmation_number_groups = { $items } العناصر المحددة في { $groups } مجموعات.
rust_delete_confirmation_selected_all_in_group = جميع العناصر المحددة في مجموعات { $groups }.
rust_trash_confirmation = هل أنت متأكد من أنك تريد نقل العناصر المحددة إلى سلة المهملات؟
rust_trash_confirmation_unsupported_volumes = ملاحظة: لا تدعم جميع وحدات التخزين الانتقال إلى سلة المهملات. بالنسبة لوحدات التخزين غير المدعومة، هذه العملية سوف تفشل دائما.
rust_trash_confirmation_number_simple = { $items } عناصر محددة.
rust_trash_confirmation_number_groups = { $items } العناصر المحددة في { $groups } مجموعات.
rust_trash_confirmation_selected_all_in_group = جميع العناصر المحددة في { $groups } مجموعات.
rust_move_confirmation = هل أنت متأكد من أنك تريد نقل العناصر المحددة؟
rust_move_confirmation_number_simple = { $items } عناصر محددة.
rust_clean_exif_confirmation = هل أنت متأكد من أنك تريد إزالة بيانات EXIF من العناصر المحددة؟
rust_clean_exif_confirmation_number_simple = { $items } عناصر محددة.
clean_exif_overwrite_files_text = استبدل الملفات
rust_optimize_video_confirmation = هل أنت متأكد من أنك تريد تحسين مقاطع الفيديو المحددة؟
rust_optimize_video_confirmation_number_simple = { $items } عناصر محددة.
rust_hardlink_confirmation = هل أنت متأكد من أنك تريد إنشاء روابط صلبة للعناصر المحددة؟
rust_hardlink_confirmation_number_simple = { $items } عناصر محددة.
rust_symlink_confirmation = هل أنت متأكد من أنك تريد إنشاء روابط رمزية للعناصر المحددة؟
rust_symlink_confirmation_number_simple = { $items } عناصر محددة.
rust_rename_confirmation = هل أنت متأكد من أنك تريد إعادة تسمية العناصر المحددة؟
rust_rename_confirmation_number_simple = { $items } عناصر محددة.
rust_cache_processed_files = تمت معالجة { $files } من ملفات التخزين المؤقت
rust_cache_entries_stats = تمت إزالة { $removed } إدخالاً من أصل { $all } إدخالاً، وتبقّى { $left }
rust_cache_size_reduced = تم تقليل حجم ملفات التخزين المؤقت بمقدار { $size }
rust_cache_time_elapsed = الوقت المنقضي: { $time }
rust_symlink_failed = فشل في إنشاء رابط رمزي لـ { $name } باتجاه { $target }، السبب { $reason }
rust_hardlink_failed = فشل إنشاء رابط صلب لـ { $name } باتجاه { $target }، السبب { $reason }

# Slint translations, but in arrays

column_selection = التحديد
column_size = الحجم
column_file_name = اسم الملف
column_path = المسار
column_modification_date = تاريخ التعديل
column_similarity = تماثل
column_dimensions = الأبعاد
column_new_dimensions = أبعاد جديدة
column_title = العنوان
column_artist = الفنان
column_year = السنة
column_bitrate = معدل
column_length = طول
column_genre = النوع
column_type_of_error = نوع الخطأ
column_symlink_name = اسم الرابط الرمزي
column_symlink_folder = مجلد الرابط الرمزي
column_destination_path = مسار الوجهة
column_current_extension = الامتداد الحالي
column_proper_extension = الامتداد الصحيح
column_fps = fps
column_codec = ترميز
column_duration = المدة
column_exif_tags = وسوم EXIF
column_new_name = اسم جديد
column_full_path = المسار الكامل
# Slint translations
ok_button = حسناً
cancel_button = إلغاء
yes_text = نعم
no_text = لا
do_you_want_to_continue = هل تريد المتابعة؟
main_window_title = Krokiet (كروكيت) - منظف البيانات
file_dialog_open = إغلاق نافذة الحوار للمتابعة
scan_button = فحص
stop_button = توقف
stop_text = توقف
select_button = حدد
move_button = نقل
delete_button = حذف
trash_button = المهملات
save_button = حفظ
sort_button = فرز
rename_button = إعادة تسمية
motto = هذا البرنامج مجاني للاستخدام وسيبقى كذلك دائمًا. راجع ترخيص MIT/GPL للحصول على التفاصيل.
unicorn = قد لا تنظر إلى وحيد القرن، ولكن وحيد القرن ينظر إليك دائما.
repository = المستودع
instruction = تعليمات
donation = تبرع
translation = الترجمة
other_apps = تطبيقات أخرى
build_info_build = الإصدار
build_info_runtime = وقت التشغيل
build_info_refresh = تحديث
build_info_diagnostic_button = معلومات التشخيص
build_info_diagnostic_title = معلومات التشخيص
build_info_library_restart_hint = * تعكس عمليات فحص المكتبات (HEIF، LibRAW، AVIF) حالتها وقت بدء التشغيل - أعد تشغيل التطبيق بعد تثبيت أو إزالة المكونات الإضافية.
build_info_test_file = ملف تجريبي
other_apps_title = تطبيقات أخرى من إعداد qarmin
other_apps_open_source_note = جميع التطبيقات المدرجة في القائمة مجانية ومفتوحة المصدر.
other_apps_open_button = فتح
other_apps_szyszka_desc = إعادة تسمية الملفات بسرعة وقوة.
other_apps_mykrut_desc = مدير ملفات لينكس بسيط وسريع وله نهج واضح.
other_apps_dcmki_viewer_desc = عارض ديكوم بسيط.
other_apps_video_thumbnailer_desc = تغليف حول مولد مصغرات الفيديو المستخدم في Czkawka.
other_apps_space_finder_desc = الباحث البسيط عن أكبر الملفات على النظام الخاص بك.
other_apps_system_info_collector_desc = يجمع استخدام RAM/CPU من نظام التشغيل ويعرضها كرسوم بيانية.
included_paths = المسارات المضمنة
excluded_paths = المسارات المستبعدة
ref = مرجع
path = المسار
tool_duplicate_files = ملفات مكررة
tool_empty_folders = مجلدات فارغة
tool_big_files = ملفات كبيرة
tool_empty_files = ملفات فارغة
tool_temporary_files = الملفات المؤقتة
tool_similar_images = صور مشابهة
tool_similar_videos = مقاطع فيديو مماثلة
tool_music_duplicates = تكرارات الموسيقى
tool_invalid_symlinks = الروابط الرمزية غير صالحة
tool_broken_files = الملفات المكسورة
tool_bad_extensions = ملحقات سيئة
tool_bad_names = أسماء سيئة
tool_video_optimizer = مُحسِّن الفيديو
tool_exif_remover = مزيل EXIF
sort_by_full_name = الترتيب حسب الاسم الكامل
sort_by_focus = الترتيب حسب التركيز
sort_reverse = عكس الترتيب
selection_all = حدد الكل
selection_deselect_all = إلغاء تحديد الكل
selection_invert_selection = عكس التحديد
selection_invert_selection_in_group = عكس التحديد في المجموعة
selection_the_biggest_size = حدد أكبر حجم
selection_the_biggest_resolution = حدد أكبر دقة
selection_the_smallest_size = حدد أصغر حجم
selection_the_smallest_resolution = حدد أصغر دقة
selection_newest = حدد الأحدث
selection_oldest = حدد الأقدم
selection_shortest_path = اختر أقصر مسار
selection_longest_path = اختر أطول مسار
selection_all_except_biggest_size = حدد الكل باستثناء أكبر حجم
selection_all_except_smallest_size = تحديد الكل باستثناء أصغر حجم
selection_all_except_biggest_resolution = حدد الكل باستثناء أكبر دقة
selection_all_except_smallest_resolution = حدد الكل باستثناء أصغر دقة
selection_all_except_newest = حدد الكل باستثناء الأحدث
selection_all_except_oldest = حدد الكل باستثناء الأقدم
selection_all_except_longest_path = حدد الكل باستثناء أطول مسار
selection_all_except_shortest_path = حدد الكل باستثناء أقصر مسار
selection_custom_select_unselect = تحديد مخصص/إلغاء تحديد
stage_current = المرحلة الحالية:
stage_all = جميع المراحل:
subsettings = الإعدادات الفرعية
subsettings_images_hash_size = حجم التجزئة
subsettings_images_resize_algorithm = خوارزمية تغيير الحجم
subsettings_images_ignore_same_size = تجاهل الصور بنفس الحجم
subsettings_images_ignore_same_resolution = تجاهل الصور بنفس الدقة
subsettings_images_max_difference = الفرق الأقصى
subsettings_images_geometric_invariance = ثبات هندسي
subsettings_images_duplicates_hash_type = نوع التجزئة
subsettings_duplicates_check_method = طريقة التحقق
subsettings_duplicates_name_case_sensitive = حساس لحالة الأحرف (طرق الاسم فقط)
subsettings_biggest_files_sub_method = الطريقة
subsettings_biggest_files_sub_number_of_files = عدد الملفات
option_search_mode_biggest = الاكبر
option_search_mode_smallest = الأصغر حجماً
option_music_method_tags = الوسوم
option_music_method_fingerprint = بصمة
option_check_method_hash = التجزئة
option_check_method_size = الحجم
option_check_method_name = الاسم
option_check_method_size_and_name = الحجم والاسم
option_crop_detect_letterbox = الأشرطة السوداء
option_crop_detect_motion = حركة
option_crop_detect_none = لا
option_video_crop_type_black_bars = الأشرطة السوداء
option_video_crop_type_static_content = المحتوى الثابت
option_video_optimizer_mode_crop = اقتصاص
option_video_optimizer_mode_transcode = إعادة الترميز
option_noise_reduction_none = لا
option_noise_reduction_hqdn3d = hqdn3d (سريع)
subsettings_videos_max_difference = الفرق الأقصى
subsettings_videos_ignore_same_size = تجاهل مقاطع الفيديو بنفس الحجم
subsettings_videos_ignore_same_resolution = تجاهل مقاطع الفيديو بنفس الدقة
subsettings_videos_audio_check_content = مقارنة حسب بصمة الصوت
subsettings_videos_audio_preset = إعدادات مسبقة سريعة
subsettings_videos_audio_preset_custom = مخصص
subsettings_videos_audio_preset_identical = مقاطع فيديو متطابقة
subsettings_videos_audio_preset_clip = مقطع ضمن فيديو أطول
subsettings_videos_audio_preset_similar = محتوى مشابه
subsettings_videos_visual_preset = إعدادات مسبقة سريعة
subsettings_videos_visual_preset_custom = مخصص
subsettings_videos_visual_preset_near_identical = شبه متطابق
subsettings_videos_visual_preset_similar = مشابه
subsettings_videos_visual_preset_movies = الأفلام (تخطي طويل)
subsettings_videos_duration_tolerance_pct = هامش التسامح في المدة [%]
subsettings_videos_min_matching_windows = الحد الأدنى لمطابقة النوافذ
subsettings_videos_subclip_min_match = الحد الأدنى لمطابقة المقطع الفرعي
subsettings_videos_audio_similarity_percent = الشبه [%]
subsettings_videos_audio_length_ratio = نسبة الطول الأدنى (أقصر/أطول)
subsettings_videos_audio_min_duration_seconds = الحد الأدنى لمدة الملف [s]
subsettings_videos_audio_maximum_difference = الحد الأقصى لفرق الصوت
subsettings_music_audio_check_type = نوع التحقق من الصوت
subsettings_music_approximate_comparison = مقارنة العلامات التقريبية
subsettings_music_compared_tags = مقارنة العلامات
subsettings_music_title = العنوان
subsettings_music_artist = الفنان
subsettings_music_bitrate = معدل
subsettings_music_genre = النوع
subsettings_music_year = السنة
subsettings_music_length = طول
subsettings_music_max_difference = الفرق الأقصى
subsettings_music_minimal_fragment_duration = الحد الأدنى من مدة الشظايا
subsettings_music_compare_fingerprints_only_with_similar_titles = مقارنة داخل مجموعات من العناوين المتشابهة
subsettings_broken_files_type = نوع الملفات المراد التحقق منها
subsettings_empty_files_type = أنواع الملفات الإضافية للعثور عليها
subsettings_empty_files_zero_byte_content = ملفات مليئة ببايتات فارغة
subsettings_empty_files_zero_byte_content_hint = أيضاً ابحث عن ملفات غير فارغة يحتوي محتواها الكامل على بايتات فارغة (0x00).
subsettings_empty_files_non_printable_content = ملفات مليئة بأحرف غير قابلة للطباعة
subsettings_empty_files_non_printable_content_hint = أيضاً ابحث عن ملفات غير فارغة تتكون فقط من أحرف ASCII غير قابلة للطباعة: null, space, tab, CR, LF, VT, FF.
subsettings_broken_files_audio = الصوت
subsettings_broken_files_pdf = بي دي إف
subsettings_broken_files_archive = أرشيف
subsettings_broken_files_image = صورة
subsettings_broken_files_video_ffprobe = فيديو (ffprobe)
subsettings_broken_files_video_ffprobe_info = التحقق السريع من الفيديو باستخدام ffprobe (التحقق من الترويسة).
subsettings_broken_files_video_ffmpeg = فيديو (ffmpeg)
subsettings_broken_files_video_ffmpeg_info = التحقق العميق من الفيديو باستخدام ffmpeg (فك الترميز الكامل). بطيء جدا وقد يكتشف أخطاء دقيقة جدًا حتى إذا كان الملف يعمل بشكل جيد.
subsettings_broken_files_font = الخط
subsettings_broken_files_markup = الترميز (JSON/XML/TOML)
subsettings_bad_names_issues = فحص أسماء الملفات
subsettings_bad_names_uppercase_extension = امتداد بأحرف كبيرة
subsettings_bad_names_uppercase_extension_hint = يجد الملفات التي تحتوي على حروف كبيرة في الامتداد (مثل .JPG، .Mp3) ويقترح النسخة الصغيرة
subsettings_bad_names_emoji_used = إيموجي في الاسم
subsettings_bad_names_emoji_used_hint = يجد الملفات التي تحتوي على أحرف تعبيرية (😀، 🎉، إلخ) في الاسم ويقترح حذفها
subsettings_bad_names_space_at_start_end = مسافات بادئة / مسافات لاحقة
subsettings_bad_names_space_at_start_end_hint = يجد الملفات التي تحتوي على مسافات في بداية أو نهاية الاسم ويقترح قصها
subsettings_bad_names_non_ascii = أحرف غير ASCII
subsettings_bad_names_non_ascii_hint = يجد أحرفًا غير ASCII (ą، ć، ñ، إلخ) ويقترح استبدالها بمكافئاتها في ASCII (a, c, n) أو إزالتها إذا لم يكن هناك تعيين
subsettings_bad_names_restricted_charset = مجموعة أحرف محدودة
subsettings_bad_names_restricted_charset_hint = يحوّل الأحرف غير ASCII إلى ASCII، ثم يجد الملفات التي تحتوي على أحرف خارج النطاق 0-9a-zA-Z والأحرف المسموح بها المحددة من قبل المستخدم
subsettings_bad_names_allowed_chars = الأحرف المسموح بها
subsettings_bad_names_remove_duplicated = أحرف مكررة
subsettings_bad_names_remove_duplicated_hint = يجد الأحرف غير الأبجدية الرقمية المتكررة المتجاورة (مثل "ملف---اسم..txt") ويقترح إزالة التكرارات
settings_global_settings = الإعدادات الشاملة
settings_dark_theme = السمة المظلمة
settings_show_only_icons = إظهار الأيقونات فقط
settings_excluded_items = البند المستبعد:
settings_allowed_extensions = الامتدادات المسموح بها:
settings_excluded_extensions = الامتدادات المستبعدة:
settings_file_size = حجم الملف (كيلوبايتات)
settings_minimum_file_size = الحد الأدنى:
settings_maximum_file_size = الحد الأقصى:
settings_recursive_search = البحث المتكرر
settings_use_cache = استخدام ذاكرة التخزين المؤقت
settings_save_as_json = حفظ ذاكرة التخزين المؤقت أيضا كملف JSON
settings_ignore_other_filesystems = تجاهل نظم الملفات الأخرى (Linux)
settings_delete_outdated_cache_entries = حذف إدخالات ذاكرة التخزين المؤقت القديمة تلقائيًا
settings_delete_outdated_cache_entries_hint = عند التفعيل، سيقوم التطبيق بالتحقق أثناء تحميل ذاكرة التخزين المؤقت (بحد أقصى مرة واحدة في الأسبوع) لمعرفة ما إذا كانت السجلات المخزنة لا تزال تشير إلى ملفات/بيانات موجودة وغير معدلة
settings_hide_hard_links = إخفاء الروابط الصلبة
settings_hide_hard_links_hint = إخفاء الروابط الصلبة للملفات نفسها في النتائج
settings_thread_number = عدد الخيوط
settings_restart_required = ---أنت بحاجة إلى إعادة تشغيل التطبيق لتطبيق التغييرات في عدد الخيوط---
settings_duplicate_image_preview = معاينة الصورة
settings_duplicate_minimal_hash_cache_size = الحجم الأدنى للملفات المخزنة مؤقتاً - هاش (KB)
settings_duplicate_use_prehash = استخدام ما قبل التجزئة
settings_duplicate_minimal_prehash_cache_size = الحجم الأدنى للملفات المخزنة مؤقتاً - بريهاش (KB)
settings_similar_images_show_image_preview = معاينة الصورة
settings_similar_videos_preview_hint = تكون المعاينة مرئية فقط عند تمكين "إنشاء الصور المصغرة"، أو عند إنشاء صورة مصغرة بالفعل.
settings_application_scale_text = تطبيق النطاق
settings_application_scale_hint_text = عند تفعيل المقياس اليدوي، يتيح لك ذلك اختيار عامل مقياس مخصص، ولكنه يعطل تمامًا القياس التلقائي المبني على كثافة النقاط (DPI) للشاشة.
settings_restart_required_scale_text = ---يجب إعادة تشغيل التطبيق لتطبيق التغييرات في المقياس---
settings_use_manual_application_scale_text = استخدم مقياس تطبيق يدوي
settings_video_thumbnails_preview = معاينة الصورة
settings_open_config_folder = فتح مجلد التكوين
settings_open_cache_folder = فتح مجلد ذاكرة التخزين المؤقت
settings_language = اللغة
settings_current_preset = الإعداد المسبق الحالي:
settings_edit_name = تحرير الاسم
settings_choose_name_for_prefix = اختر اسم البادئة
settings_save = حفظ
settings_load = تحميل
settings_reset = إعادة تعيين
settings_similar_videos_tool = أداة فيديو مشابهة
settings_video_thumbnails_clear_unused_thumbnails = حذف الصور المصغرة للفيديو غير المستخدمة التي يزيد عمرها عن 7 أيام عند بدء تشغيل التطبيق
settings_video_thumbnails_header = صورة مصغرة للفيديو
settings_video_thumbnails_generate = إنشاء صور مصغرة
settings_video_thumbnails_position = موضع الصورة المصغرة في الفيديو (%)
settings_video_thumbnails_generate_grid = إنشاء شبكة صور مصغرة بدلاً من صورة واحدة
settings_video_thumbnails_generate_grid_hint = إن إنشاء صور متعددة في شبكة أبطأ بكثير من إنشاء صورة مصغرة واحدة
settings_video_thumbnails_grid_tiles_per_side = عدد البلاطات في كل جانب من شبكة الصورة المصغرة
settings_video_thumbnails_grid_tiles_per_side_hint = عدد مربعات الصور المصغرة في كل جانب من الشبكة. على سبيل المثال، تحديد 2 ينشئ شبكة 2 × 2، مما ينتج عنه صورة مصغرة واحدة تتكون من 4 صور.
settings_similar_images_tool = أداة مشابهة للصور
settings_general_settings = الإعدادات العامة
settings_select_header_text = حدد خيارات منبثقة
settings_select_group_size_text = الحجم
settings_select_group_date_text = تاريخ التعديل
settings_select_group_resolution_text = الدقة
settings_select_group_path_text = المسار
settings_select_label_one_smallest_text = أصغر واحد
settings_select_label_one_biggest_text = واحد أكبر
settings_select_label_except_smallest_text = باستثناء أصغر
settings_select_label_except_biggest_text = باستثناء الاكبر
settings_select_label_one_oldest_text = أقدم واحد
settings_select_label_one_newest_text = واحد أحدث
settings_select_label_except_oldest_text = باستثناء الأقدم
settings_select_label_except_newest_text = باستثناء الأحدث
settings_select_label_shortest_text = أقصر
settings_select_label_longest_text = الأطول
settings_select_label_except_shortest_text = باستثناء الأقصر
settings_select_label_except_longest_text = باستثناء الأطول
settings_cache_header_text = إعدادات التخزين المؤقت
settings_clean_cache_button_text = امسح ذاكرة التخزين المؤقت القديمة
settings_settings = الإعدادات
settings_load_tabs_sizes_at_startup = تحميل أحجام علامات التبويب عند بدء التشغيل
settings_load_windows_size_at_startup = تحميل حجم النوافذ عند بدء التشغيل
settings_limit_lines_of_messages = قصر الرسائل على 500 سطر (العمل على أداة تحرير نص بطيئ)
settings_play_audio_on_scan_completion_text = تشغيل الصوت عند اكتمال المسح بنجاح
settings_audio_feature_hint_text = متاح فقط عند التجميع مع الميزة الصوتية
settings_audio_env_variable_hint_text = يمكن تغيير الصوت عن طريق تعيين متغير البيئة KROKIET_AUDIO_STOP_FILE إلى مسار ملف صوتي صالح
settings_show_notification_on_scan_completion_text = إظهار الإشعار عند اكتمال الفحص
settings_notification_feature_hint_text = متاح فقط عند التجميع مع ميزة الإشعارات
settings_notification_focused_hint_text = قد لا يظهر الإشعار كنافذة منبثقة عندما تكون نافذة التطبيق مركزة
popup_save_title = حفظ النتائج
popup_save_message = سيؤدي هذا إلى حفظ النتائج إلى 3 ملفات مختلفة
popup_rename_title = إعادة تسمية الملفات
popup_rename_single_title = إعادة تسمية الملف
popup_rename_single_same_name = الاسم الجديد هو نفس الاسم الحالي.
popup_rename_single_target_exists = يوجد بالفعل ملف بهذا الاسم في هذا المجلد.
popup_error_title = خطأ
popup_new_paths_title = أضف المسارات، مسارًا واحدًا في كل سطر
popup_move_title = نقل الملفات
popup_move_copy_checkbox = نسخ الملفات بدلاً من النقل
popup_move_preserve_folder_checkbox = الحفاظ على هيكل المجلد
popup_move_rename_on_conflict_checkbox = إعادة التسمية عند التضارب (بدلاً من تخطّي)
move_confirmation_text = هل أنت متأكد من أنك تريد نقل العناصر المحددة؟
rename_confirmation_text = هل أنت متأكد من أنك تريد إعادة تسمية العناصر المحددة؟
delete = حذف العناصر
trash = نقل العناصر إلى سلة المهملات
stopping_scan = إيقاف الفحص، الرجاء الانتظار...
searching = يبحث...
subsettings_videos_crop_detect = كشف الأشرطة السوداء
subsettings_videos_skip_forward_amount = تخطي المدة [s]
subsettings_videos_vid_hash_duration = مدة تجزئة الفيديو
subsettings_videos_window_count = عدد النافذة
settings_cache_number_size_text = حجم ملفات التخزين المؤقت: { $size }، عدد الملفات: { $number }
settings_video_thumbnails_number_size_text = حجم الصور المصغرة للفيديو: { $size }، عدد الملفات: { $number }
settings_log_number_size_text = حجم ملفات السجل: { $size }، عدد الملفات: { $number }
popup_clean_cache_title_text = مسح ذاكرة التخزين المؤقت القديمة
popup_clean_cache_confirmation_text = هل أنت متأكد من أنك تريد مسح إدخالات ذاكرة التخزين المؤقت القديمة؟ سيؤدي ذلك إلى إزالة إدخالات ذاكرة التخزين المؤقت للملفات التي لم تعد موجودة أو تم تعديلها.
popup_clean_cache_progress_text = جاري معالجة ملف ذاكرة التخزين المؤقت:
popup_clean_cache_current_file_text = الملف الحالي:
popup_clean_cache_file_progress_text = التقدم الحالي للملف:
popup_clean_cache_overall_progress_text = التقدم العام:
popup_clean_cache_stopped_by_user_text = تم إيقاف تنظيف ذاكرة التخزين المؤقت بواسطة المستخدم
popup_clean_cache_finished_text = تم تنظيف ذاكرة التخزين المؤقت بنجاح!
popup_clean_cache_error_details_text = تفاصيل الخطأ:
popup_clean_cache_files_with_errors = ملفات بها أخطاء:
subsettings_video_optimizer_mode = وضع
subsettings_video_optimizer_crop_type = نوع الاقتصاص
subsettings_video_optimizer_black_pixel_threshold = حد البكسل الأسود
subsettings_video_optimizer_black_pixel_threshold_hint = القيمة القصوى لـ RGB لكل قناة بكسل لاعتبارها سوداء (0-128). القيمة الافتراضية: 20
subsettings_video_optimizer_black_bar_min_percentage = الحد الأدنى لنسبة الشريط الأسود
subsettings_video_optimizer_black_bar_min_percentage_hint = الحد الأدنى لنسبة بكسلات سوداء في صف/عمود لاعتبارها شريطًا أسود (50-100). القيمة الافتراضية: 90
subsettings_video_optimizer_max_samples = الحد الأقصى للعينات
subsettings_video_optimizer_max_samples_hint = الحد الأقصى لعدد الإطارات لتحليلها لكل فيديو (5-1000). القيمة الافتراضية: 60
subsettings_video_optimizer_min_crop_size = الحد الأدنى لحجم الاقتصاص
subsettings_video_optimizer_min_crop_size_hint = يتم تجاهل عمليات القص الأصغر. القيمة الافتراضية: 5
subsettings_video_optimizer_video_codec = فيديو كودك
subsettings_video_optimizer_excluded_codecs = محذوفات الترميز
subsettings_video_optimizer_video_quality = جودة الفيديو (CRF)
subsettings_video_optimizer_noise_reduction = تقليل الضوضاء
subsettings_video_optimizer_noise_reduction_strength = قوة NR
subsettings_video_optimizer_use_custom_command = استخدام أمر FFmpeg مخصص
subsettings_video_optimizer_custom_command_hint = الأمر مع { "{PATH}" } لملف الإدخال. يتم إلحاق مسار الإخراج تلقائياً.
subsettings_video_optimizer_hardware_encoder = الترميز باستخدام العتاد
subsettings_video_optimizer_generate_template = إنشاء قالب
subsettings_reset = إعادة تعيين
subsettings_exif_ignored_tags_text = الوسوم المتجاهلة:
subsettings_exif_ignored_tags_hint_text = قائمة مفرغة بفواصل من العلامات المستبعدة من الفحص (مثل GPS، Thumbnail). بعض العلامات، مثل ImageWidth في ملفات TIFF، مخفية لمنع كسر الصورة.
subsettings_temporary_files_extensions_text = الملحقات:
subsettings_temporary_files_extensions_hint_text = قائمة مفصولة بفواصل من الملحقات/لاحقة تعامل كمؤقتة (مثال: .tmp,.bak,~). إعادة تعيين الإعدادات الافتراضية المدمجة.
clean_button_text = تنظيف
clean_text = تنظيف بيانات EXIF
clean_confirmation_text = هل أنت متأكد من أنك تريد إزالة بيانات EXIF من العناصر المحددة؟
crop_videos_text = قص الفيديو
crop_video_confirmation_text = هل أنت متأكد من أنك تريد اقتطاف الفيديوهات المحددة؟
crop_reencode_video_text = إعادة ترميز الفيديو
reencode_videos_text = إعادة ترميز الفيديوهات
optimize_button_text = تحسين
optimize_confirmation_text = هل أنت متأكد من أنك تريد إعادة ترميز الفيديوهات المحددة؟
optimize_fail_if_bigger_text = فشل إذا كان الملف المحسن أكبر
optimize_overwrite_files_text = استبدل الملفات
optimize_limit_video_size_text = حدّ حجم الفيديو
optimize_max_width_text = الحد الأقصى للعرض:
optimize_max_height_text = الحد الأقصى للارتفاع:
optimize_crf_hint = القيمة الأدنى = جودة أفضل. 0 يكاد يكون بدون فقدان، 51 أسوأ من ذلك. موصى به: 18-28.
optimize_noise_reduction_hint = تقليل الضوضاء قد يزيد وقت الترميز زيادة كبيرة.
optimize_noise_reduction_strength_hint = 1 = الحد الأدنى لتقليل الضجيج، 10 = الحد الأقصى لتقليل الضجيج.
hardlink_button_text = رابط صلب
hardlink_text = إنشاء روابط صلبة
hardlink_confirmation_text = هل أنت متأكد من أنك تريد إنشاء روابط صلبة للعناصر المحددة؟
softlink_button_text = رابط رمزي
softlink_text = إنشاء روابط رمزية
softlink_confirmation_text = هل أنت متأكد من أنك تريد إنشاء روابط رمزية (symlinks) للعناصر المحددة؟
popup_custom_select_title_text = تحديد مخصص / إلغاء التحديد
popup_custom_select_button_text = حدد
popup_custom_unselect_button_text = إلغاء التحديد
popup_custom_column_name_header_text = العمود
popup_custom_filter_value_header_text = قيمة الفلتر (حرف بديل / regex)
popup_custom_case_sensitive_text = حساسية الحالة
popup_custom_leave_one_in_group_text = حدد جميع العناصر باستثناء عنصر واحد، في كل مجموعة
popup_custom_hint_str_text = أعمدة النص: أحرف البدل *name* /home/* *.rs
popup_custom_hint_int_text = الحجم [KB] / الأعمدة الرقمية: >= 2048 < 512 = 0 (المشغلين: >= <= > < =)
popup_custom_hint_date_text = أعمدة التاريخ: DD-MM-YYYY أو YYY-MM-DD, الوقت الاختياري HH:MM:SS على سبيل المثال >= 2020-01-01 أو < 31-12-2022 23:59:59
popup_custom_save_restore_text = حفظ واستعادة بيانات الفلتر
compare_button_text = قارن
compare_back_text = الرجوع
compare_prev_group_text = المجموعة السابقة
compare_next_group_text = المجموعة التالية
compare_swap_text = تبديل
compare_info_text = معلومات
compare_diff_text = فرق
compare_split_text = تقسيم
compare_overlay_text = تراكب
compare_loading_title_text = الرجاء الانتظار...
compare_cancel_text = إلغاء
compare_resolution_mismatch_text = دقة مختلفة - قد تكون نتائج المقارنة غير دقيقة
compare_computing_diff_text = جارٍ حساب الفرق...
compare_info_file_name_text = اسم الملف
compare_info_path_text = المسار
compare_info_size_text = الحجم
compare_info_resolution_text = الدقة
compare_info_modification_date_text = تاريخ التعديل
compare_loading_images_text = تحميل الصور...
compare_cancelling_text = إلغاء، يرجى الانتظار...
context_menu_open_item_text = فتح عنصر
context_menu_open_parent_text = فتح المجلد الأصلي
context_menu_remove_from_results_text = إزالة من النتائج
context_menu_remove_all_from_folder_text = إزالة الكل من المجلد
context_menu_remove_all_from_folder_recursive_text = إزالة الكل من المجلد (متكرر)
context_menu_select_all_from_folder_text = حدد الكل من المجلد
context_menu_select_all_from_folder_recursive_text = حدد الكل من المجلد (متكرر)
context_menu_exclude_parent_folder_text = استبعاد المجلد الأصلي
context_menu_exclude_item_text = استبعاد البند
context_menu_copy_file_name_text = نسخ اسم الملف
context_menu_copy_parent_folder_path_text = نسخ المجلد الأصلي
context_menu_copy_full_path_text = نسخ المسار الكامل
context_menu_rename_text = إعادة تسمية
