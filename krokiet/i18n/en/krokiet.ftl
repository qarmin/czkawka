...
searching = Searching...
subsettings_videos_engine = Search engine
subsettings_videos_engine_hint = VidDupFinder is the classic engine. Perceptual uses frame-level pHash (no ffmpeg required). Audio uses Chromaprint fingerprinting.
subsettings_videos_perceptual_preset = Perceptual preset
subsettings_videos_perceptual_preset_hint = Controls how many frames are hashed and the minimum number of matching frames required. Faster presets are less accurate.
subsettings_videos_audio_preset = Audio preset
subsettings_videos_audio_preset_hint = Controls how much audio is fingerprinted per file. Faster presets analyse less audio.
