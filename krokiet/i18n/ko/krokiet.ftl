...
rust_renamed_file = "{ $old_name }"을 "{ $new_name }"으로 이름이 변경되었습니다
...
rust_found_big_files = { $time } 내에 크기 { $size }의 큰 파일 { $items_found }개를 찾았습니다
...
rust_trash_confirmation_number_simple = { $items } 항목 선택됨.
rust_trash_confirmation_number_groups = { $items } 개가 선택되어 있는 { $groups } 그룹입니다.
rust_trash_confirmation_selected_all_in_group = { $groups } 그룹에서 선택된 모든 항목.
...
translation = 번역
other_apps = 다른 앱
build_info_build = 구축하다
build_info_runtime = 실행 시간
...
other_apps_video_thumbnailer_desc = Czkawka에서 사용되는 비디오 썸네일 생성기를 감싸는 래퍼 코드.
...
selection_all_except_biggest_size = 가장 큰 사이즈를 제외하고 모두 선택하세요
selection_all_except_smallest_size = 가장 작은 크기를 제외한 모든 항목을 선택하세요
selection_all_except_biggest_resolution = 가장 큰 해상도를 제외하고 모두 선택
selection_all_except_smallest_resolution = 가장 작은 해상도를 제외하고 모두 선택합니다
selection_all_except_newest = 가장 최신인 파일 제외하고 모두 선택
selection_all_except_oldest = 가장 오래된 파일 제외하고 모두 선택
selection_all_except_longest_path = 선택 전부 제외 가장 긴 경로
selection_all_except_shortest_path = 선택 모두 제외 짧은 경로
...
option_search_mode_biggest = 큰 파일
option_search_mode_smallest = 작은 파일
...
subsettings_videos_audio_preset = 빠른 설정 미리보기
subsettings_videos_audio_preset_custom = 맞춤 제작
subsettings_videos_audio_preset_identical = 동일한 동영상
subsettings_videos_audio_preset_clip = 더 긴 비디오에서 일부만 잘라내기
subsettings_videos_audio_preset_similar = 유사한 내용
subsettings_videos_visual_preset = 빠른 설정 미리보기
subsettings_videos_visual_preset_custom = 맞춤 제작
subsettings_videos_visual_preset_near_identical = 거의 동일한
subsettings_videos_visual_preset_similar = 유사한
subsettings_videos_visual_preset_movies = 영화 (길게 건너뛰기)
subsettings_videos_duration_tolerance_pct = 허용 오차 [지속 시간, %]
subsettings_videos_min_matching_windows = Min 창 최소화
subsettings_videos_subclip_min_match = 최소 일치 서브클립
...
subsettings_videos_audio_maximum_difference = 최대 음향 차이
...
subsettings_empty_files_zero_byte_content_hint = 또한, 내용이 모두 널 바이트(0x00)로만 이루어진 비어 있지 않은 파일도 찾아야 합니다.
subsettings_empty_files_non_printable_content = 인쇄할 수 없는 문자로 채워진 파일들
subsettings_empty_files_non_printable_content_hint = 또한, 다음과 같은 비어 있지 않고, 오직 인쇄할 수 없는 ASCII 문자(null, 공백, 탭, CR, LF, VT, FF)로만 구성된 파일도 찾아주세요.
...
subsettings_broken_files_video_ffprobe = 비디오 (ffprobe)
subsettings_broken_files_video_ffprobe_info = ffprobe를 사용하여 영상 파일을 빠르게 확인합니다 (헤더 유효성 검사).
subsettings_broken_files_video_ffmpeg = 영상 (ffmpeg)
subsettings_broken_files_video_ffmpeg_info = ffmpeg를 사용하여 비디오를 깊이 분석합니다 (전체 디코딩). 매우 느리며, 파일이 정상적으로 재생되더라도 세세한 오류를 감지할 수 있습니다.
...
settings_similar_videos_preview_hint = 미리보기는 "미리보기 생성" 옵션이 활성화되었거나, 미리보기가 이미 생성된 경우에만 표시됩니다.
...
settings_select_label_one_biggest_text = 가장 큰 것 중 하나
settings_select_label_except_smallest_text = 가장 작은 것을 제외하고
settings_select_label_except_biggest_text = 가장 큰 것 제외하고
settings_select_label_one_oldest_text = 가장 오래된 것 중 하나
settings_select_label_one_newest_text = 가장 최신 모델
settings_select_label_except_oldest_text = 가장 오래된 것 제외하고
settings_select_label_except_newest_text = 최신 제품 제외하고
settings_select_label_shortest_text = 가장 짧은
settings_select_label_longest_text = 가장 긴
settings_select_label_except_shortest_text = 가장 짧은 경우를 제외하고는
settings_select_label_except_longest_text = 가장 긴 것 제외하고
...
settings_limit_lines_of_messages = 메시지를 500줄로 제한하세요(TextEdit 위젯이 느릴 때의 대안)
...
subsettings_video_optimizer_noise_reduction = 소음 감소
subsettings_video_optimizer_noise_reduction_strength = NR 강점
subsettings_video_optimizer_use_custom_command = 사용자 정의 FFmpeg 명령어를 사용하세요
...
subsettings_temporary_files_extensions_text = 확장 프로그램:
...
popup_custom_hint_str_text = 텍스트 필드: 와일드카드 *name* /home/* *.rs
...
context_menu_open_item_text = 열기 항목
...
context_menu_remove_all_from_folder_recursive_text = 폴더 내 모든 파일 삭제 (재귀적)
context_menu_select_all_from_folder_text = 폴더에서 모두 선택
context_menu_select_all_from_folder_recursive_text = 폴더 내 모든 항목 선택 (재귀적)
...
context_menu_copy_parent_folder_path_text = 부모 폴더 복사
