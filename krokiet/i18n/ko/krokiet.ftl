...
rust_found_similar_videos = {$items_found}개의 비슷한 동영상 파일을 {$groups} 그룹에서 {$time}时间内找到了{$items_found}个相似的视频文件在{$groups}组用时{$time}
...
rust_found_bad_extensions = {$items_found}개의 파일에 이상한 확장자가 있어서 {$time}内马查找具有不良扩展名的文件{ $items_found}个在{ $time}
rust_found_duplicate_files = {$items_found}개의 중복 파일을 {$groups} 그룹으로 분류하여/{$size} 크기로發現 {$items_found}개의 중복 파일이 {$groups} 그룹으로 분류되어 {$size} 크기에 {$time} 내에找到了 {$items_found}개的重复文件，在{$groups}个组中，大小为{$size}，在{$time}内发现并整理完成
