...
motto = Bu programın kullanımı ücretsizdir ve her zaman ücretsiz olacaktır. Detaylar için MIT/GPL lisansını inceleyebilirsiniz.
...
build_info_build = İnşa et
build_info_runtime = Çalışma zamanı
build_info_refresh = Yenile
build_info_diagnostic_button = Tanı Bilgileri
build_info_diagnostic_title = Tanı Bilgileri
build_info_library_restart_hint = * Kütüphane sorguları (HEIF, LibRAW, AVIF) başlangıçtaki durumu yansıtır - eklentileri yükledikten veya kaldırdıktan sonra uygulamayı yeniden başlatın.
build_info_test_file = Test Dosyası
other_apps_title = qarmin'den diğer uygulamalar
other_apps_open_source_note = Listelenen tüm uygulamalar ücretsizdir ve açık kaynaklıdır.
other_apps_open_button = Açık
other_apps_szyszka_desc = Hızlı ve güçlü bir dosya yeniden adlandırma aracı.
other_apps_mykrut_desc = Basit, hızlı ve belirli bir amaca yönelik olarak tasarlanmış bir Linux dosya yöneticisi.
other_apps_dcmki_viewer_desc = Basit bir DICOM görüntüleyici.
other_apps_video_thumbnailer_desc = Czkawka'da kullanılan video önizleme üreticisini saran bir katman.
other_apps_space_finder_desc = Sisteminizdeki en büyük dosyaları kolayca bulmanızı sağlayan basit bir araç.
other_apps_system_info_collector_desc = İşletim sisteminden RAM/CPU kullanım verilerini toplar ve bunları grafikler halinde gösterir.
