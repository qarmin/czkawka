...
pub(crate) fn connect_show_confirmation(app: &MainWindow, shared_models: Arc<Mutex<SharedModels>>) {
    ...
    app.on_request_setup_action_popup(move |popup_request: PopupRequest| {
        ...
        match popup_request {
            ...
            PopupRequest::Exclude => {
                let mut base = flk!("rust_exclude_confirmation");
                base.push_str(format!("\n{}", flk!("rust_exclude_confirmation_number_simple", items = res.checked_items_number)).as_str());
                translation.set_exclude_confirmation_text(base.into());
            }
