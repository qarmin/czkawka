    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            let params = SimilarVideosParameters::new(
                ...
                sd.custom_settings.similar_videos_audio_minimum_segment_duration,
                sd.custom_settings.similar_videos_audio_maximum_difference as f64,
                sd.custom_settings.similar_videos_audio_max_duration_diff_ratio as f64,
            );
...
fn write_similar_videos_results(
    ...
    if let Some(critical) = messages_data.critical {
        ...
        let result_message = flk!("rust_found_similar_videos", items_found = items_found, groups = groups, time = scanning_time_str);
        if !stopped_search && sd.basic_settings.show_notification_on_scan_completion {
            crate::notification_manager::send_scan_completed_notification("Similar Videos", &result_message);
        }
