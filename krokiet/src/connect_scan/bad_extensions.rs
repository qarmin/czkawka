...
pub(crate) fn scan_bad_extensions(a: Weak<MainWindow>, sd: ScanData) {
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            sd.shared_models.lock().expect("Mutex poisoned").shared_bad_extensions_state = Some(tool);
...
fn write_bad_extensions_results(app: &MainWindow, vector: Vec<BadFileEntry>, messages_data: MessagesData, info: bad_extensions::Info, sd: ScanData, stopped_search: bool) {
    ...
    if let Some(critical) = messages_data.critical {
        ...
        let result_message = flk!("rust_found_bad_extensions", items_found = items_found, time = scanning_time_str);
        if !stopped_search && sd.basic_settings.show_notification_on_scan_completion {
            crate::notification_manager::send_scan_completed_notification("Bad Extensions", &result_message);
        }
