...
fn write_empty_folders_results(app: &MainWindow, vector: Vec<FolderEntry>, messages_data: MessagesData, info: empty_folder::Info, sd: ScanData, stopped_search: bool) {
    ...
    if let Some(critical) = messages_data.critical {
        ...
        let result_message = flk!("rust_found_empty_folders", items_found = items_found, time = scanning_time_str);
        if !stopped_search && sd.basic_settings.show_notification_on_scan_completion {
            crate::notification_manager::send_scan_completed_notification("Empty Folders", &result_message);
        }
