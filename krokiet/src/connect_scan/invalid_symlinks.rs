...
pub(crate) fn scan_invalid_symlinks(a: Weak<MainWindow>, sd: ScanData) {
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            sd.shared_models.lock().expect("Mutex poisoned").shared_same_invalid_symlinks = Some(tool);
...
fn write_invalid_symlinks_results(app: &MainWindow, vector: Vec<SymlinksFileEntry>, messages_data: MessagesData, info: invalid_symlinks::Info, sd: ScanData, stopped_search: bool) {
    ...
    if let Some(critical) = messages_data.critical {
        ...
        let result_message = flk!("rust_found_invalid_symlinks", items_found = items_found, time = scanning_time_str);
        if !stopped_search && sd.basic_settings.show_notification_on_scan_completion {
            crate::notification_manager::send_scan_completed_notification("Invalid Symlinks", &result_message);
        }
