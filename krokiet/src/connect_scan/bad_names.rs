...
fn write_bad_names_results(app: &MainWindow, vector: Vec<BadNameEntry>, messages_data: MessagesData, info: bad_names::Info, sd: ScanData, stopped_search: bool) {
    ...
    if let Some(critical) = messages_data.critical {
        ...
        let result_message = flk!("rust_found_bad_names", items_found = items_found, time = scanning_time_str);
        if !stopped_search && sd.basic_settings.show_notification_on_scan_completion {
            crate::notification_manager::send_scan_completed_notification("Bad Names", &result_message);
        }
