...
fn write_similar_music_results(
    ...
    if let Some(critical) = messages_data.critical {
        ...
        let result_message = flk!("rust_found_similar_music_files", items_found = items_found, groups = groups, time = scanning_time_str);
        if !stopped_search && sd.basic_settings.show_notification_on_scan_completion {
            crate::notification_manager::send_scan_completed_notification("Similar Music", &result_message);
        }
