...
class ToolSettingsPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        title = QLabel(tr("tool-settings-title"))
        font = title.font()
        font.setBold(True)
        title.setFont(font)
    ...
    def _create_duplicate_panel(self) -> QWidget:
        ...
        self._dup_method.addItems(["Hash", "Size", "Name", "Fuzzy Name", "Size and Name"])
        method_map = {CheckingMethod.HASH: 0, CheckingMethod.SIZE: 1,
                      CheckingMethod.NAME: 2, CheckingMethod.FUZZY_NAME: 3,
                      CheckingMethod.SIZE_NAME: 4}
        ...
        layout.addRow(tr("subsettings-check-method"), self._dup_method)
        ...
        layout.addRow(tr("subsettings-hash-type"), self._dup_hash)

        # Name similarity threshold (for fuzzy name mode)
        from PySide6.QtWidgets import QSlider, QLabel, QHBoxLayout
        from PySide6.QtCore import Qt
        threshold_widget = QWidget()
        threshold_layout = QHBoxLayout(threshold_widget)
        threshold_layout.setContentsMargins(0, 0, 0, 0)
        self._dup_threshold_slider = QSlider(Qt.Horizontal)
        self._dup_threshold_slider.setRange(50, 100)
        self._dup_threshold_slider.setValue(int(self._ts.dup_name_similarity_threshold * 100))
        self._dup_threshold_label = QLabel(f"{self._ts.dup_name_similarity_threshold:.0%}")
    ...
    def _on_dup_method_changed(self, idx):
        methods = [CheckingMethod.HASH, CheckingMethod.SIZE,
                   CheckingMethod.NAME, CheckingMethod.FUZZY_NAME,
                   CheckingMethod.SIZE_NAME]
        self._ts.dup_check_method = methods[idx]
        self.settings_changed.emit()

    def _on_dup_threshold_changed(self, value):
        self._ts.dup_name_similarity_threshold = value / 100.0
        self._dup_threshold_label.setText(f"{value}%")
        self.settings_changed.emit()
    ...
    def _create_similar_images_panel(self) -> QWidget:
        ...
        layout.addRow(tr("subsettings-hash-size"), self._img_hash_size)
        ...
        layout.addRow(tr("subsettings-resize-algorithm"), self._img_filter)
        ...
        layout.addRow(tr("subsettings-image-hash-type"), self._img_hash_alg)

        # Ignore same size
        self._img_ignore_size = QCheckBox(tr("subsettings-ignore-same-size"))
    ...
    def _create_similar_videos_panel(self) -> QWidget:
        ...
        # Ignore same size
        vid_ignore = QCheckBox(tr("subsettings-ignore-same-size"))
        ...
        layout.addRow(tr("subsettings-crop-detect"), self._vid_crop)
        ...
        layout.addRow(tr("subsettings-skip-forward"), skip_layout)
    ...
    def _create_similar_music_panel(self) -> QWidget:
        ...
        layout.addRow(tr("subsettings-audio-check-type"), self._music_method)

        # Tags group
        self._tags_group = QGroupBox(tr("subsettings-tag-matching"))
        tags_layout = QVBoxLayout(self._tags_group)

        self._music_approx = QCheckBox(tr("subsettings-approximate-comparison"))
        ...
        # Fingerprint group
        self._fp_group = QGroupBox(tr("subsettings-fingerprint-matching"))
        fp_layout = QFormLayout(self._fp_group)

        fp_similar = QCheckBox(tr("subsettings-compare-similar-titles"))
    ...
    def _create_big_files_panel(self) -> QWidget:
        ...
        self._big_mode.addItems([tr("subsettings-the-biggest"), tr("subsettings-the-smallest")])
        ...
        layout.addRow(tr("subsettings-method"), self._big_mode)
        ...
        layout.addRow(tr("subsettings-number-of-files"), self._big_count)
    ...
    def _create_bad_names_panel(self) -> QWidget:
        panel = QWidget()
        layout = QVBoxLayout(panel)
        layout.addWidget(QLabel(tr("subsettings-check-for")))

        checks = [
            ("bad_names_uppercase_ext", tr("subsettings-uppercase-ext"),
             tr("subsettings-uppercase-ext-hint")),
            ("bad_names_emoji", tr("subsettings-emoji"),
             tr("subsettings-emoji-hint")),
            ("bad_names_space", tr("subsettings-space"),
             tr("subsettings-space-hint")),
            ("bad_names_non_ascii", tr("subsettings-non-ascii"),
             tr("subsettings-non-ascii-hint")),
            ("bad_names_remove_duplicated", tr("subsettings-remove-duplicated"),
             tr("subsettings-remove-duplicated-hint")),
        ]
        ...
        # Restricted charset
        layout.addWidget(QLabel(tr("subsettings-restricted-charset")))
        self._bad_names_charset = QLineEdit(self._ts.bad_names_restricted_charset)
        self._bad_names_charset.setPlaceholderText(tr("subsettings-restricted-charset-hint"))
    ...
    def _create_exif_panel(self) -> QWidget:
        ...
        self._exif_tags.setPlaceholderText(tr("subsettings-ignored-exif-tags-hint"))
        self._exif_tags.textChanged.connect(
            lambda t: setattr(self._ts, 'exif_ignored_tags', t)
        )
        layout.addRow(tr("subsettings-ignored-exif-tags"), self._exif_tags)
    ...
    def _create_video_optimizer_panel(self) -> QWidget:
        ...
        # Crop settings
        self._crop_group = QGroupBox(tr("subsettings-crop-settings"))
        ...
        crop_layout.addRow(tr("subsettings-crop-type"), self._vo_crop_type)
        ...
        crop_layout.addRow(tr("subsettings-black-pixel-threshold"), self._vo_threshold)
        ...
        crop_layout.addRow(tr("subsettings-black-bar-min-pct"), self._vo_bar_pct)
        ...
        crop_layout.addRow(tr("subsettings-max-samples"), self._vo_samples)
        ...
        # Transcode settings
        self._transcode_group = QGroupBox(tr("subsettings-transcode-settings"))
        ...
        tc_layout.addRow(tr("subsettings-excluded-codecs"), self._vo_codecs)
        ...
        tc_layout.addRow(tr("subsettings-target-codec"), self._vo_codec)
        ...
        tc_layout.addRow(tr("subsettings-quality"), self._vo_quality)

        self._vo_fail_bigger = QCheckBox(tr("subsettings-fail-if-bigger"))
