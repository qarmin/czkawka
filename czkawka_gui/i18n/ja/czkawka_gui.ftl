...
# General
general_ok_button = 了解
general_close_button = 閉じる
# Krokiet info dialog
krokiet_info_title = 廃止のお知らせ
krokiet_info_message =
    Czkawka GTK 12.0は最終版です。今後、アップデート、新機能、バグ修正の予定はありません。
    
    Czkawka GTKの多くの機能は、Krokietで利用可能です。通常、よりシンプルで、高速で、安定した形で提供されています。また、Krokietには、GTK版では実現できなかった新しい機能や改善が追加されています。
    
    もし、まだCzkawka GTKをご利用の場合、Krokietへの移行は比較的容易です。なぜなら、インターフェースが類似しており、依存関係が少なく、より優れたクロスプラットフォーム対応を実現しているからです。
    
    追伸：このメッセージは一度だけ表示されるはずです。もし再度表示される場合は、環境変数CZKAWKA_DONT_ANNOY_MEを任意の非空の値に設定してください。
# Main window
...
same_music_seconds_label = フラグメントの最小継続時間（秒）
...
duplicate_case_sensitive_name_tooltip =
    有効にすると、大文字・小文字も含めて完全に同じ名前のレコードのみをグループ化します。例: Żołd <-> Żołd
    
    このオプションを無効にすると、各文字の大文字・小文字を区別せずに名前をグループ化します。例: żoŁD <-> Żołd
...
duplicate_check_method_tooltip = Czkawkaは、今のところ以下の3種類の方法で重複を見つけることができます:  名前 - 同じ名前のファイルを検索します。  サイズ - 同じサイズのファイルを探します。  ハッシュ - 同じ内容のファイルを探します。ファイルをハッシュ化して比較することにより重複を見つけます。このモードは、重複を見つけるための最も安全な方法です。このツールはキャッシュを多用するので、同じデータの2回目以降のスキャンは最初の時よりずっと速くなるはずです。
image_hash_size_tooltip = チェックした画像はそれぞれ特別なハッシュを生成し、そのハッシュを比較したときに差が小さいほど、この画像は類似していることを意味します。  8のハッシュサイズは、オリジナルに少ししか似ていない画像を見つけるのにかなり適しています。しかし、1000枚を超えるような大きな画像では、誤検出が多くなるため、より大きなハッシュサイズを使用することをお勧めします。  16はデフォルトのハッシュサイズであり、少しでも類似した画像を見つけることとハッシュの衝突を少なくすることの間でかなり良い妥協点です。  32と64のハッシュは非常に類似した画像しか見つけられませんが、誤検出はほとんどありません（アルファチャンネルのある一部の画像を除いて）。
image_resize_filter_tooltip = 画像のハッシュを計算するために、ライブラリはまず画像のサイズを必ず変更します。  どのアルゴリズムを選択したかによって、ハッシュを計算するために使用される画像は少し違って見えるかもしれません。  最も高速なアルゴリズムは Nearest ですが、最も悪い結果を出すのも Nearest です。16x16のハッシュサイズで低品質の場合、それが明らかになることはないので、デフォルトは Nearest です。  8x8のハッシュサイズでは、より良い画像群を得るために、Nearestとは異なるアルゴリズムを使用することが推奨されます。
image_hash_alg_tooltip = ハッシュの計算方法は、多くのアルゴリズムの中からユーザーが選択することができます。  それぞれ長所と短所があり、画像によって良い結果が出る場合もあれば、悪い結果が出る場合もあります。  そのため、最適なものを見極めるには、手動でのテストが必要です。
image_geometric_invariance_tooltip = また、各画像のミラーリング/フリップとオプションで回転したバリエーションを比較します。これによりマッチングが改善されますが、ハッシュ時間が増加します。
big_files_mode_combobox_tooltip = 最小/最大のファイルを検索できます
big_files_mode_label = チェックされたファイル
big_files_mode_smallest_combo_box = 最も小さい
big_files_mode_biggest_combo_box = 最も大きい
...
upper_manual_add_included_button_tooltip =
    ...
    一度に複数のパスを追加するには、セミコロン(;)で区切ってください。
    
    /home/roman;/home/rozkaz と入力すると、/home/roman と /home/rozkaz の2つのディレクトリが追加されます
...
upper_manual_add_excluded_button_tooltip =
    ...
    /home/roman;/home/krokiet と入力すると、/home/roman と /home/keokiet の2つのディレクトリが追加されます
...
upper_allowed_extensions_tooltip =
    ...
    使用例: ".exe, IMAGE, VIDEO, .rar, 7z" - これは画像（jpg、pngなど）、ビデオ（avi、mp4など）、exe、rar、7zファイルがスキャンされることを意味します。
upper_excluded_extensions_tooltip = スキャンで無視される無効なファイルの一覧です。  許可された拡張子と無効化された拡張子の両方を使用する場合、この拡張子の方が優先度が高いので、ファイルはチェックされません。
upper_excluded_items_tooltip = 除外項目には *ワイルドカードを含んでおり、カンマで区切ってください。 これはExcluded Pathsよりも遅いため、注意して使用してください。
upper_excluded_items = 除外するアイテム:
upper_allowed_extensions = 許可される拡張子:
upper_excluded_extensions = 無効な拡張子:
# Popovers
popover_select_all = すべて選択
popover_unselect_all = すべて選択解除
popover_reverse = 選択を逆にする
popover_select_all_except_shortest_path = 最短パス以外のすべてを選択
popover_select_all_except_longest_path = 最長パス以外のすべてを選択
...
popover_custom_name_check_button_entry_tooltip =
    ...
    使用例:
    /usr/ping/pong.txt は *ong* で見つけることができます
popover_custom_regex_check_button_entry_tooltip =
    指定した正規表現でレコードを選択することができます。
    
    このモードでは、検索される文字列はパスと名前になります。
    
    使用例:
    /usr/bin/ziemniak.txt は /ziem[a-z]+ で検索できます。
    
    これはRustのデフォルトの正規表現実装を使用しているので、詳しくは https://docs.rs/regex を参照してください。
popover_custom_case_sensitive_check_button_tooltip = 大文字小文字を区別する検出を有効にします。  /home/* を無効にすると、/Home/roman と /home/roman の両方が検出されます。
popover_custom_not_all_check_button_tooltip =
    グループ内のすべてのレコードを選択できないようにします。
    
    ほとんどの状況では、元のファイルと重複ファイルの両方を削除するのではなく、少なくとも1つは残しておきたいと考えられるため、これはデフォルトで有効になっています。
...
popover_custom_case_sensitive_check_button = 大文字小文字を区別する
...
popover_sort_folder_name = フォルダ名
popover_sort_full_name = フルネーム
...
bottom_symlink_button_tooltip = シンボリックリンクを作成します。 グループ内の2つ以上の結果が選択されている場合にのみ機能します。 最初の結果は変更されず、2番目以降の結果が最初の結果にシンボリックリンクされます。
bottom_hardlink_button_tooltip = ハードリンクを作成します。 グループ内の2つ以上の結果が選択されている場合にのみ機能します。 最初の結果は変更されず、2番目以降の結果が最初の結果にハードリンクされます。
...
bottom_move_button_tooltip =
    選択したフォルダにファイルを移動します。
    ディレクトリツリーを維持せずに、すべてのファイルをフォルダにコピーします。
...
progress_stop_additional_message = 停止をリクエストしました
# About Window
...
about_other_apps_video_thumbnailer_desc = Czkawkaで使用されているビデオサムネイル生成ツールのラッパー。
...
header_krokiet_button_tooltip = Krokietを試してみてください - 新しいバージョンと改善されたバージョン!
# Krokiet promo dialog
krokiet_promo_title = Krokietをご紹介します!
krokiet_promo_message = こんにちは、勇敢なCzkawkaユーザーの皆さん！  フォースがあなたと共にありますね。しかし、残念ながらKrokietは一緒ではありません。Krokietは、より新しく、高速で、軽量で、そして何よりも（アプリが美しくあり得るという前提で）ずっと魅力的な、重複ファイルクリーナーです。  Krokietには、Czkawkaで人々が気に入っていたすべての機能が含まれています。完全に無料で、オープンソースであり、独特でシンプルなUI（多くの人に賞賛される一方で、嫌われることもあります）、多くの新機能の導入、依存関係の削減、そしてさまざまなプラットフォームでの信頼性の向上を実現しています。  もし見逃してしまったかもしれませんが、Krokietよりもさらに新しいアプリ、Cediniaがあります。これは主にAndroidデバイスとタッチ操作用に設計されています。  Czkawka GTKは私たちに貢献してくれましたが、その役目は終わりました。
krokiet_promo_link_download = Krokiet/Cedinia をダウンロード
krokiet_promo_link_project = プロジェクトページ
...
settings_use_rust_preview_tooltip = GTKプレビューはいくらかの場合において高速で多くのフォーマットをサポートしているが、全く逆となる場合もある。  プレビューの読み込みに問題がある場合、この設定を変更してみるとよい。  Linux以外の環境では、gtk-pixbufが常に有効とは限らず、無効にすることによりいくらかの画像のプレビューが読み込まれないため、このオプションの使用が推奨される。
...
settings_load_at_start_button_tooltip = 起動時にファイルから設定を読み込みます。  このオプションが無効の場合、デフォルトの設定が使用されます。
...
settings_save_also_as_json_button_tooltip = キャッシュを人間にも読みやすいJSON形式で保存します。この内容は編集可能です。バイナリ形式のキャッシュ（bin拡張子のもの）がない場合、このファイルから自動的にキャッシュが読み込まれます。
...
settings_multiple_clear_cache_button_tooltip = 古いキャッシュエントリを手動でクリアします。 自動クリアが無効の場合にのみ使用する必要があります。
...
settings_duplicates_minimal_size_entry_tooltip = キャッシュされるファイルの最小サイズを設定します。  値を小さくするとキャッシュが生成されるレコードが増え検索が高速化しますが、キャッシュの読み込みと保存が遅くなります。
settings_duplicates_prehash_checkbutton_tooltip = プレハッシュ(ファイルの一部から計算したハッシュ) のキャッシュを有効にし、重複していない検索結果をより早く捨てられるようにします。  いくつかの場面では低速化の要因になりうるので、この機能はデフォルトでは無効になっています。  数十万・数百万のファイルをスキャンする場合には、検索を何倍も高速化できるため使用を強く推奨します。
...
settings_folder_cache_open_tooltip = キャッシュを持つtxtファイルが保存されているフォルダを開きます。  これらのファイルを変更すると不正な結果を表示することがありますが、パスなどを変更することで、大量のファイルを別の場所に移動する際の時間を短縮することができます。  このファイルをコンピュータ間でコピーすることで、再度ファイルをスキャンするときの時間を節約できます（もちろん、コンピュータのディレクトリ構造が似ている場合に限り）。  キャッシュに問題がある場合、このファイルを削除することができます。アプリは自動的にそれらを再生成します。
settings_folder_settings_open_tooltip = Czkawkaの設定ファイルが保存されているフォルダを開きます。  警告: 手動で変更すると、ワークフローが壊れる可能性があります。
settings_folder_cache_open = キャッシュフォルダを開く
...
compute_found_images = 類似画像を{ $number_files }枚見つけました。これらは{ $number_groups }グループに分かれ、{ $time }で検索しました
compute_found_videos = { $number_files }個の類似ビデオを{ $number_groups }グループ中で{ $time }に見つけました
compute_found_music = { $number_files }個の類似音楽ファイルを{ $number_groups }グループ中で{ $time }に発見しました
...
about_window_motto = このプログラムは無料で自由に使用でき、これからもそうであり続けます。
...
hard_sym_invalid_selection_label_2 = これらのファイルをハードリンク/シンボリックリンクできるようにするには、グループ内の少なくとも2つの結果を選択する必要があります。
...
cache_clear_similar_videos_title = 類似したビデオのキャッシュをクリア中
