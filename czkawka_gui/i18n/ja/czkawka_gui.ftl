...
upper_excluded_extensions_tooltip = スキャンで無視される無効なファイルの一覧です。\n\n許可された拡張子と無効化された拡張子の両方を使用する場合、この拡張子の方が優先度が高いので、ファイルはチェックされません。
upper_excluded_items_tooltip = 除外項目には *ワイルドカードを含んでおり、カンマで区切ってください。\nこれはExcluded Pathsよりも遅いため、注意して使用してください。
...
popover_custom_case_sensitive_check_button_tooltip = 大文字小文字を区別する検出を有効にします。\n\n/home/* を無効にすると、/Home/roman と /home/roman の両方が検出されます。
...
krokiet_promo_message = こんにちは、勇敢なCzkawkaユーザーの皆さん！\n\nフォースがあなたと共にありますね。しかし、残念ながらKrokietは一緒ではありません。Krokietは、より新しく、高速で、軽量で、そして何よりも（アプリが美しくあり得るという前提で）ずっと魅力的な、重複ファイルクリーナーです。\n\nKrokietには、Czkawkaで人々が気に入っていたすべての機能が含まれています。完全に無料で、オープンソースであり、独特でシンプルなUI（多くの人に賞賛される一方で、嫌われることもあります）、多くの新機能の導入、依存関係の削減、そしてさまざまなプラットフォームでの信頼性の向上を実現しています。\n\nもし見逃してしまったかもしれませんが、Krokietよりもさらに新しいアプリ、Cediniaがあります。これは主にAndroidデバイスとタッチ操作用に設計されています。\n\nCzkawka GTKは私たちに貢献してくれましたが、その役目は終わりました。
...
settings_load_at_start_button_tooltip = 起動時にファイルから設定を読み込みます。\n\nこのオプションが無効の場合、デフォルトの設定が使用されます。
