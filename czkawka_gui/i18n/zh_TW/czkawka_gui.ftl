...
image_hash_size_tooltip =
    每張經過檢查的圖片都會產生一個特殊的雜湊值，這個雜湊值可以與其他任何圖片的雜湊值進行比較。如果這些雜湊值之間的差異很小，則意味著這些圖片是相似的。
    
    使用 8 位元的雜湊大小非常適合找出與原圖稍微相似的圖片。但如果圖片集合很大（超過 1000 張），則可能會產生大量的誤判，因此在這種情況下建議使用更大的雜湊大小。

    16 位元是預設的雜湊大小，這是一個在找出稍微相似的圖片和減少雜湊碰撞之間的良好妥協。
    
    使用 32 或 64 位元的雜湊大小只會找到非常相似的圖片，幾乎不會產生誤判（可能的例外是一些帶有 alpha 透明通道的圖片）。
image_resize_filter_tooltip =
    要計算圖片的雜湊值，函式庫首先必須對其進行縮放。
    
    根據所選的演算法，用於計算雜湊值的結果圖片可能會看起來略有不同。

    最快的演算法是 Nearest，但它也提供了最差的結果。它是預設啟用的，因為在 16x16 的雜湊大小下，較低的品質實際上並不明顯。
    
    若使用 8x8 的雜湊大小，建議使用不同於 Nearest 的演算法，以獲得更好的圖片分組
...
check_button_general_same_size_tooltip = 從結果中忽略大小相同的檔案 - 通常這些是 1:1 的重複
...
# Progress window
progress_scanning_general_file = 正在掃描 { $file_number } 檔案
progress_scanning_extension_of_files = 檢查副檔名 { $file_checked }/{ $all_files } 檔案
progress_scanning_broken_files = 正在檢查 { $file_checked }/{ $all_files } 檔案
progress_scanning_video = 雜湊 { $file_checked }/{ $all_files } 影片
progress_scanning_image = 雜湊 { $file_checked }/{ $all_files } 影像
progress_comparing_image_hashes = 比較 { $file_checked }/{ $all_files } 影像雜湊
progress_scanning_music_tags_end = 對比標籤 { $file_checked }/{ $all_files } 音樂檔案
progress_scanning_music_tags = 正在讀取標籤 { $file_checked }/{ $all_files } 音樂檔案
progress_scanning_music_content_end = 比較指紋 { $file_checked }/{ $all_files } 音樂檔案
progress_scanning_music_content = 正在計算指紋 { $file_checked }/{ $all_files } 音樂檔案
progress_scanning_empty_folders = 正在掃描 { $folder_number } 資料夾
progress_scanning_size = 正在掃描檔案大小 { $file_number }
progress_scanning_size_name = 掃描檔名和大小 { $file_number }
progress_scanning_name = 正在掃描 { $file_number } 檔案的名稱
progress_analyzed_partial_hash = 分析了 { $file_checked }/{ $all_files } 檔案的部分雜湊
progress_analyzed_full_hash = 分析了 { $file_checked }/{ $all_files } 檔案的完整雜湊值
...
save_results_to_file = 結果儲存到檔案 { $name }
