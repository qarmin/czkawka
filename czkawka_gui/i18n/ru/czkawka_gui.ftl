...
compute_found_duplicates_hash_size = Найдено дубликатов: { $number_files } (групп: { $number_groups }), размер: { $size }, **поиск занял { $time }**
compute_found_duplicates_name = Найдено: { $number_files } дубликат(и/ів) (групп: { $number_groups }), **поиск занял { $time }**
