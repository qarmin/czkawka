...
As extensões permitidas devem ser separadas por vírgulas. Por padrão, todas as extensões estão disponíveis.

Existem os macros que permitem a inclusão de vários tipos de arquivos específicos. Por exemplo, de IMAGEM, VÍDEO, MÚSICA, TEXTO de uma só vez.

Um exemplo de utilização ‘.exe, IMAGEM, VÍDEO, .rar, 7z’, significa que as imagens com as extensões ‘.jpg’ e ‘.png’, os vídeos com as extensões ‘.avi’ e ’.mp4’, os arquivos com as extensões ‘.exe’, ‘.rar’ e ‘.7z’ serão verificados.
...
progress_scanning_broken_files = Pesquisando por arquivos { $file_checked } de { $all_files }
progress_scanning_video = Criando o código ‘hash’ dos arquivos de vídeo { $file_checked } de { $all_files }
progress_scanning_image = Criando o código ‘hash’ dos arquivos de imagem { $file_checked } de { $all_files }
...
progress_scanning_music_content = Calculando a impressão digital dos arquivos de música { $file_checked } de { $all_files }
...
progress_analyzed_partial_hash = Calculando o ‘hash’ parcial dos arquivos { $file_checked } de { $all_files }
progress_analyzed_full_hash = Calculando o ‘hash’ completo dos arquivos { $file_checked } de { $all_files }
...
about_window_motto = Este programa é e sempre será de uso gratuito.
