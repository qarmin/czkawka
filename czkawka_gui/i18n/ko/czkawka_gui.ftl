...
same_music_tooltip =
    음악 파일 내용 기반 유사도 검색은 다음 설정으로 조정할 수 있습니다:
    
    - 유사하다고 판단할 수 있는 최소 조각 시간
    - 비교되는 두 조각 간에 허용되는 최대 차이
    
    좋은 결과를 얻는 핵심은 이 두 값을 상황에 맞게 조합하는 것입니다.
    
    최소 시간 5초 + 최대 차이 1.0으로 설정하면 거의 동일한 조각을 찾습니다.
    반면 시간 20초 + 최대 차이 6.0은 리믹스/라이브 버전 등을 찾는 데 효과적입니다.
    
    기본적으로 모든 음악 파일을 서로 비교하므로 파일이 많으면 시간이 오래 걸릴 수 있습니다. 따라서 일반적으로 기준 폴더를 사용하여 서로 비교할 파일을 지정하는 것이 좋습니다(같은 수의 파일 기준, 기준 폴더를 사용하면 지문 비교가 최소 4배 빨라집니다).
...
duplicate_hash_type_tooltip =
    ...
    Blake3 - 암호화 해시 함수입니다. 매우 빠르기 때문에 기본값으로 설정되어 있습니다.
    
    CRC32 - 간단한 해시 함수입니다. Blake3보다 빠르지만, 매우 드물게 충돌이 발생할 수 있습니다.
    
    XXH3 - 해시 품질과 성능 면에서 Blake3와 매우 유사하지만 암호화 해시는 아닙니다. 따라서 두 모드는 서로 대체하여 사용할 수 있습니다.
...
image_hash_size_tooltip =
    확인된 각 이미지마다 서로 비교할 수 있는 특별한 해시가 생성되며, 해시 간 차이가 작으면 이미지가 유사하다는 의미입니다.
    
    해시 크기 8은 원본과 조금만 유사한 이미지를 찾는 데 적합합니다. 이미지가 많은 경우(1000개 이상) 거짓 양성이 많이 발생할 수 있으므로, 이 경우 더 큰 해시 크기를 권장합니다.
    
    기본값인 16은 조금 유사한 이미지 탐지와 해시 충돌 최소화 사이의 적절한 절충안입니다.
    
    32와 64는 매우 유사한 이미지만 찾지만, 거짓 양성이 거의 없습니다(알파 채널이 있는 일부 이미지 제외).
image_resize_filter_tooltip =
    이미지 해시를 계산하려면 라이브러리가 먼저 이미지 크기를 조정해야 합니다.
    
    선택한 알고리즘에 따라 해시 계산에 사용되는 이미지가 조금씩 달라집니다.
    
    가장 빠르지만 결과가 가장 나쁜 알고리즘은 Nearest입니다. 16x16 해시 크기에서는 품질 저하가 잘 드러나지 않아 기본값으로 설정되어 있습니다.
    
    8x8 해시 크기에서는 더 나은 이미지 그룹화를 위해 Nearest 이외의 알고리즘을 사용하는 것이 좋습니다.
...
image_geometric_invariance_tooltip = 각 이미지의 거울 이미지/반전 이미지, 그리고 선택적으로 회전된 이미지 변형도 함께 비교합니다. 이렇게 하면 정확도를 높일 수 있지만, 해싱 처리 시간이 늘어납니다.
big_files_mode_combobox_tooltip = 가장 큰 파일 또는 가장 작은 파일을 찾을 수 있습니다
big_files_mode_label = 찾을 파일
big_files_mode_smallest_combo_box = 가장 작은 파일
big_files_mode_biggest_combo_box = 가장 큰 파일
...
main_notebook_similar_images = 유사한 이미지
main_notebook_similar_videos = 유사한 동영상
...
main_tree_view_column_dimensions = 해상도
...
main_check_box_broken_files_video_tooltip = ffmpeg/ffprobe를 사용하여 동영상 파일의 유효성을 검사합니다. 상당히 느리며, 파일이 잘 재생되더라도 사소한 형식 오류까지 감지할 수 있습니다.
check_button_general_same_size = 같은 파일 크기 무시
...
upper_manual_add_included_button_tooltip =
    ...
    여러 경로를 입력하려면 ';'로 구분하십시오.
    
    '/home/roman;/home/rozkaz'를 입력하면 '/home/roman'과 '/home/rozkaz' 두 디렉터리가 추가됩니다
...
upper_manual_add_excluded_button_tooltip =
    ...
    '/home/roman;/home/krokiet'를 입력하면 '/home/roman'과 '/home/krokiet' 두 디렉터리가 추가됩니다
...
upper_excluded_items_tooltip =
    제외 항목에는 * 와일드카드가 포함되어야 하며, 항목은 쉼표로 구분합니다.
    제외 경로보다 느리므로 주의해서 사용하십시오.
...
popover_select_all_except_shortest_path = 가장 짧은 경로 제외하고 모두 선택
popover_select_all_except_longest_path = 가장 긴 경로 제외하고 모두 선택
...
popover_custom_not_all_check_button_tooltip =
    ...
    이 옵션은 기본적으로 켜져 있습니다. 대부분의 경우 원본과 중복 파일을 전부 삭제하기보다는 최소한 하나의 파일은 남겨두고 싶을 것이기 때문입니다.
    
    경고: 이 설정은 이미 수동으로 그룹의 모든 항목을 선택해 둔 경우에는 작동하지 않습니다.
...
about_other_apps_button = 다른 앱
about_other_apps_dialog_title = qarmin에서 제공하는 다른 애플리케이션
about_other_apps_open_source_note = 위에 나열된 모든 애플리케이션은 무료이며 오픈 소스입니다.
about_other_apps_open_button = 열기
about_other_apps_szyszka_desc = 빠르고 강력한 파일 이름 변경 도구.
about_other_apps_mykrut_desc = 간단하고 빠르며, 특정한 철학을 가진 리눅스 파일 관리자.
about_other_apps_dcmki_viewer_desc = 간단한 DICOM 뷰어.
about_other_apps_video_thumbnailer_desc = Czkawka에서 사용되는 동영상 썸네일 생성기를 감싸는 래퍼입니다.
about_other_apps_space_finder_desc = 시스템 내에서 가장 큰 파일들을 간단하게 찾아주는 프로그램입니다.
about_other_apps_system_info_collector_desc = OS에서 RAM/CPU 사용량을 수집하여 그래프 형태로 보여줍니다.
# Header
header_setting_button_tooltip = 설정창을 엽니다.
header_about_button_tooltip = 이 앱에 대한 정보창을 엽니다.
header_krokiet_button_tooltip = Krokiet을 사용해 보세요 - 새롭고 개선된 버전입니다!
# Krokiet promo dialog
krokiet_promo_title = Krokiet을 소개합니다!
...
settings_use_rust_preview_tooltip =
    GTK 미리보기가 더 빠르고 더 많은 형식을 지원할 때도 있지만, 정반대인 경우도 있습니다.
    
    미리보기 로드에 문제가 있다면 이 설정을 변경해 보십시오.
    
    리눅스가 아닌 시스템에서는 gtk-pixbuf를 항상 사용할 수 있는 것은 아니므로 이 옵션을 사용하는 것이 좋습니다. 이 옵션을 끄면 일부 이미지의 미리보기가 로드되지 않을 수 있습니다.
...
settings_multiple_delete_outdated_cache_checkbutton_tooltip =
    더 이상 존재하지 않는 파일에 대한 캐시 항목을 삭제합니다.
    
    이 옵션이 켜져 있으면, 프로그램은 캐시를 불러올 때 모든 항목이 유효한 파일을 가리키는지 확인합니다(손상된 항목은 무시됩니다).
    
    이 옵션을 끄면 외장 드라이브의 파일을 스캔할 때 도움이 됩니다. 해당 파일들의 캐시 항목이 다음 스캔에서 삭제되지 않기 때문입니다.
    
    캐시에 수십만 개의 항목이 있는 경우 이 옵션을 켜는 것이 좋습니다. 스캔 시작/종료 시 캐시 로드/저장 속도가 빨라집니다.
settings_notebook_general = 일반
settings_notebook_duplicates = 중복 파일
settings_notebook_images = 유사한 이미지
settings_notebook_videos = 유사한 동영상
...
settings_duplicates_hide_hard_link_button_tooltip =
    같은 데이터를 가리키는(하드 링크된) 파일이 여러 개인 경우 하나만 남기고 숨깁니다.
    
    예: 디스크에 특정 데이터에 하드 링크된 파일 7개와, 같은 데이터를 가지지만 inode가 다른 파일 1개가 있는 경우, 중복 파일 찾기에서는 고유 파일 1개와 하드 링크된 파일 중 1개만 표시됩니다.
settings_duplicates_minimal_size_entry_tooltip =
    캐시에 추가되기 위한 최소 파일 크기를 설정합니다.
    
    이 값이 작을수록 더 많은 항목이 캐시에 저장됩니다. 검색은 빨라지지만, 캐시 로드/저장은 느려집니다.
settings_duplicates_prehash_checkbutton_tooltip =
    사전 해시(파일의 일부만으로 계산되는 해시) 캐싱을 활성화하여, 중복이 아닌 파일을 더 일찍 결과에서 제외합니다.
    
    일부 상황에서는 속도 저하를 일으킬 수 있어 기본적으로 꺼져 있습니다.
    
    수십만~수백만 개의 파일을 스캔할 때는 검색 속도가 몇 배 빨라질 수 있으므로 사용을 적극 권장합니다.
...
compute_found_duplicates_hash_size = { $number_groups }개 그룹에서 중복 파일 { $number_files }개를 { $time } 만에 찾았습니다 (차지 용량: { $size })
compute_found_duplicates_name = { $number_groups }개 그룹에서 중복 파일 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_empty_folders = 빈 폴더 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_empty_files = 빈 파일 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_big_files = 큰 파일 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_temporary_files = 임시 파일 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_images = { $number_groups }개 그룹에서 유사한 이미지 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_videos = { $number_groups }개 그룹에서 유사한 동영상 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_music = { $number_groups }개 그룹에서 유사한 음악 파일 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_invalid_symlinks = 잘못된 심볼릭 링크 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_broken_files = 손상된 파일 { $number_files }개를 { $time } 만에 찾았습니다
compute_found_bad_extensions = 확장자에 문제가 있는 파일 { $number_files }개를 { $time } 만에 찾았습니다
# Progress window
progress_current_stage = 현재 단계:{ "  " }
progress_all_stages = 전체 단계:{ "  " }
# Saving loading 
saving_loading_saving_success = 설정을 { $name } 파일에 저장했습니다.
saving_loading_saving_failure = 설정 데이터를 { $name } 파일에 저장하지 못했습니다. 이유: { $reason }.
saving_loading_reset_configuration = 현재 설정이 초기화됨.
saving_loading_loading_success = 앱 설정 불러오기 성공.
saving_loading_no_config_file = 설정 파일이 없어 기본 설정을 사용합니다.
...
text_view_errors = 오류
about_window_motto = 이 프로그램은 무료이며, 앞으로도 항상 무료일 것입니다.
krokiet_new_app = 이 GTK 버전의 Czkawka는 버전 12를 끝으로 더 이상 개발되지 않습니다. 새로운 기능과 활발한 개발을 원하시면 더 안정적이고 성능이 좋은 Krokiet을 사용하십시오.
# Various dialog
dialogs_ask_next_time = 다음에도 묻기
symlink_failed = { $name }을(를) { $target }(으)로 심볼릭 링크하지 못했습니다. 이유: { $reason }
delete_title_dialog = 삭제 확인
delete_question_label = 정말로 파일을 삭제하시겠습니까?
delete_all_files_in_group_title = 그룹의 모든 파일 삭제 확인
delete_all_files_in_group_label1 = 일부 그룹 내에 있는 모든 파일이 선택되어 있습니다.
delete_all_files_in_group_label2 = 정말로 해당 파일을 모두 삭제하시겠습니까?
delete_items_label = { $items }개의 파일이 삭제됩니다.
delete_items_groups_label = { $groups }개 그룹에서 { $items }개의 파일이 삭제됩니다.
hardlink_failed = { $name }을(를) { $target }(으)로 하드 링크하지 못했습니다. 이유: { $reason }
...
hard_sym_link_label = 정말로 해당 파일들을 링크하시겠습니까?
...
search_not_choosing_any_music = 오류: 최소한 하나의 음악 검색 방식을 선택해야 합니다.
search_not_choosing_any_broken_files = 오류: 최소한 하나 이상의 검사할 손상 파일 유형을 선택해야 합니다.
...
cache_clear_similar_videos_title = 유사한 동영상 캐시 정리
...
compare_groups_number = 그룹 { $current_group } / { $all_groups } (이미지 { $images_in_group }개)
